...
class Pedido extends Model
{
    ...
    protected $fillable = [
        ...
        'confirmado',
        'tipo_pedido', // OPEN MARKET, STANDING ORDER
    ...
    {
        return $this->hasMany('\yura\Modelos\Envio', 'id_pedido')->orderBy('id_envio');
    }
                    ...
                    foreach ($esp_emp->detalles as $det_esp) {
                        if ($esp_emp->empaque->f_empaque == 'T') {  // pedido de tallos sueltos en mallas
                            ...
                            if ($this->tipo_especificacion == 'T') {    // flor tinturada
                                ...
                                //dump($ramos .' '. getPrecioByDetEsp($det_ped->precio, $det_esp->id_detalle_especificacionempaque).' '.$det_esp->id_detalle_especificacionempaque);
                                $precio_final =  $ramos * getPrecioByDetEsp($det_ped->precio, $det_esp->id_detalle_especificacionempaque);
                                $r += $precio_final;
                            }
                        }
    ...
    public function getRamosPedido()
    {
        $ramos = 0;
        ...
        foreach ($this->detalles as $det_ped)
            foreach ($det_ped->cliente_especificacion->especificacion->especificacionesEmpaque as $esp_emp)
                foreach ($esp_emp->detalles as $det_esp_emp)
                    $ramos += $det_ped->cantidad * $det_esp_emp->cantidad;

        return $ramos;
    }

    public function getMarcaciones(){
        return DB::table('detallepedido_datoexportacion as m')
            ->join('detalle_pedido as dp', 'dp.id_detalle_pedido', '=', 'm.id_detalle_pedido')
            ->select('m.valor')->distinct()
            ->where('dp.id_pedido', $this->id_pedido)
    ...
    public function getDatosExportacion(){
        return DB::table('detallepedido_datoexportacion as m')
            ->join('detalle_pedido as dp', 'dp.id_detalle_pedido', '=', 'm.id_detalle_pedido')
            ->join('dato_exportacion as de', 'de.id_dato_exportacion', '=', 'm.id_dato_exportacion')
            ->select('de.nombre', 'm.id_dato_exportacion')->distinct()
            ->where('dp.id_pedido', $this->id_pedido)
            ->get();
    }
}
