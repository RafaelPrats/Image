...
class ClasificacionVerde extends Model
{
    ...
    public function total_tallos_recepcion()
    {
        $r = DB::table('desglose_recepcion as dr')
            ...
            ->where('r.fecha_ingreso', 'like', $this->fecha_ingreso . '%')
            ->where('r.estado', 1)
            ->where('dr.estado', 1)
            ->get()[0]->cant;
        return $r > 0 ? $r : 0;
    ...
    {
        $r = DB::table('detalle_clasificacion_verde')
            ->select(DB::raw('sum(cantidad_ramos) as cant'))
            ->where('id_clasificacion_verde', $this->id_clasificacion_verde)
            ->where('estado', 1)
            ->get()[0]->cant;
        return $r > 0 ? $r : 0;
    ...
    {
        return 100 - porcentaje($this->total_tallos(), $this->total_tallos_recepcion(), 1);
    }

    public function desechoByVariedad($variedad)
    {
        $total_tallos_recepcionByVariedad = $this->total_tallos_recepcionByVariedad($variedad);
        if ($total_tallos_recepcionByVariedad > 0)
            return round(100 - (($this->tallos_x_variedad($variedad) * 100) / $total_tallos_recepcionByVariedad), 2);
        else return 0;
    }

    public function getCalibre()
    {
        $getTotalRamosEstandar = $this->getTotalRamosEstandar();
        if ($getTotalRamosEstandar > 0) {
            return round($this->total_tallos() / $getTotalRamosEstandar, 2);
        } else
            return 0;
    }
    ...
    {
        $getTotalRamosEstandarByVariedad = $this->getTotalRamosEstandarByVariedad($variedad);
        if ($getTotalRamosEstandarByVariedad > 0)
            return round($this->tallos_x_variedad($variedad) / $getTotalRamosEstandarByVariedad, 2);
        else
            return 0;
    }
    ...
    {
        $l = DB::table('detalle_clasificacion_verde as d')
            ->select('d.id_clasificacion_unitaria','u.nombre')->distinct()
    ...
    {
        $getCantidadHorasTrabajoByVariedad = $this->getCantidadHorasTrabajoByVariedad($variedad);
        if (count($this->detalles) > 0 && $this->personal > 0 && $getCantidadHorasTrabajoByVariedad > 0) {
            $r = $this->total_tallos_rendimientoByVariedad($variedad) / $this->personal;
            $r = $r / $getCantidadHorasTrabajoByVariedad;
    ...
    function getTotalRamosEstandar()
    {
        ...
        $listado = DB::table('detalle_clasificacion_verde as d')
            ...
            ->select('d.id_variedad', 'd.id_clasificacion_unitaria','v.nombre')->distinct()
