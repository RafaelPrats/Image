...
class DetalleAplicacion extends Model
{
    ...
    public function getTipoParametros()
    {
        $tipo = DB::table('parametro_detalle_aplicacion')
            ->select('tipo')->distinct()
            ->where('id_detalle_aplicacion', $this->id_detalle_aplicacion)
            ->get();
        if (count($tipo) > 0)
            return count($tipo) == 1 ? $tipo[0]->tipo : -1;
        else
            return '';
    }
}
