...
function espacios($texto) //---- Elimina espacios de los extremos de la cadena y deja un solo espacio entre palabras
{
    ...
    foreach ($arr_variantes as $variante => $substituto) {
        while ($txt != str_replace($variante, $substituto, $txt))
            $txt = str_replace($variante, $substituto, $txt);
    }
...
{
    return Variedad::where('estado', '=', 1)->orderBy('nombre')->get();
}

function getVariedadesByPlanta($p, $formato = 'option', $select = '')
{
    $p = Planta::find($p);
    if ($formato == 'option') {
        $r = '';
        foreach ($p->variedades->where('estado', 1)->sortBy('orden') as $v) {
            $selected = $v->id_variedad == $select ? 'selected' : '';
...
function getLongitudesByPlanta($p, $formato = 'option', $select = 1)
{
    if ($formato == 'option') {
        $r = '';
        ...
        foreach ($longitudes as $l) {
            $r .= '<option value="' . $l->id_proy_longitudes . '">' . $l->nombre . '</option>';
        }
        return $r;
    } else {
        return $p->variedades;
...
{
    return UnidadMedida::where('estado', '=', 1)->orderBy('siglas')->get();
}
...
function getSemanaByCodigo($codigo)
{
    $r = Semana::where('codigo', $codigo)
        ->get()
        ->first();
    return $r;
}
...
{
    $r = Semana::where('fecha_inicial', '<=', $fecha)
        ->where('fecha_final', '>=', $fecha)
        ->get()
        ->first();
...
{
    $r = Semana::where('id_variedad', '=', $variedad)
        ->where('fecha_inicial', '<=', $fecha)
        ->where('fecha_final', '>=', $fecha)
        ->get()
        ->first();
...
function getDetalleEspecificacion($id_especificacion)
{
    ...
    $arrData = [];
    foreach ($data->especificacionesEmpaque as $espEmp) foreach ($espEmp->detalles as $det)
        $arrData[] = [
...
{
    $r = 0;
    foreach (getPedido($id_pedido)->detalles as $det_ped) foreach ($det_ped->cliente_especificacion->especificacion->especificacionesEmpaque as $esp_emp) foreach ($esp_emp->detalles as $det_esp)
        $r++;
    return $r;
}
...
{
    $r = 0;
    foreach (Especificacion::find($id_especificacion)->especificacionesEmpaque as $esp_emp) foreach ($esp_emp->detalles as $det_esp)
        $r++;
    return $r;
}
...
function getCantidadRamosDistribuidosForCB(
    $fecha,
    $variedad,
    $presentacion,
    $tallos_x_ramo,
    ...
    $r = DB::table('distribucion_posco')
        ->select(
            DB::raw('sum(cantidad) as cantidad')
        )
        ->where('fecha', '=', $fecha)
        ->where('id_empaque', '=', $presentacion)
        ->where('tallos_x_ramo', '=', $tallos_x_ramo)
...
function tieneDistribucionesPendientes(
    $fecha,
    $presentacion,
    $tallos_x_ramos,
    $longitud_ramo,
    ...
    $pedidos = DB::table('pedido as p')
        ...
        ->where('dee.tallos_x_ramos', '=', $tallos_x_ramos)
        ->where('dee.longitud_ramo', '=', $longitud_ramo)
        ->where('dee.id_unidad_medida', '=', $unidad_medida)
        ->get()->pluck('id_pedido')->toArray();
    $r = true;
    foreach ($pedidos as $p) {
        if (!isPedidoDistribuido($p))
            $r = false;
    }
    return $r;
}
...
function getCantidadRamosPedidosForCB(
    ...
    $query = DB::table('pedido as p')
        ...
        ->where('dee.id_empaque_p', '=', $presentacion);
    if ($tallos_x_ramos !== '')
        $query = $query->where('dee.tallos_x_ramos', '=', $tallos_x_ramos);
    if ($longitud_ramo !== '')
        $query = $query->where('dee.longitud_ramo', '=', $longitud_ramo);
    if ($unidad_medida !== '')
        $query = $query->where('dee.id_unidad_medida', '=', $unidad_medida);
    $query = $query->groupBy(
        'dp.id_detalle_pedido',
        'dp.cantidad',
        'dee.id_detalle_especificacionempaque',
    ...
    foreach ($query as $q) {
        ...
        $actual += isset($getRamosXCajaModificado) ? ($getRamosXCajaModificado->cantidad * $q->piezas) : $q->ramos;
    }

    $variedad_model = Variedad::find($variedad);
    ...
    $modificaciones_solidas = PedidoModificacion::join('detalle_especificacionempaque as dee', 'dee.id_detalle_especificacionempaque', '=', 'pedido_modificacion.id_detalle_especificacionempaque')
        ...
        ->where('dee.id_variedad', '=', $variedad)
        ->where('pedido_modificacion.fecha_anterior_pedido', $fecha)
        ->where('pedido_modificacion.usar', 1)
        ->where('dee.id_empaque_p', '=', $presentacion);
    if ($tallos_x_ramos !== '')
        $modificaciones_solidas = $modificaciones_solidas->where('dee.tallos_x_ramos', '=', $tallos_x_ramos);
    if ($longitud_ramo !== '')
        $modificaciones_solidas = $modificaciones_solidas->where('dee.longitud_ramo', '=', $longitud_ramo);
    if ($unidad_medida !== '')
        $modificaciones_solidas = $modificaciones_solidas->where('dee.id_unidad_medida', '=', $unidad_medida);
    $modificaciones_solidas = $modificaciones_solidas->get();
    ...
    $modificaciones_mixtas = PedidoModificacion::join('detalle_especificacionempaque as dee', 'dee.id_detalle_especificacionempaque', '=', 'pedido_modificacion.id_detalle_especificacionempaque')
        ...
        ->where('dee.id_empaque_p', '=', $presentacion);
    if ($tallos_x_ramos != '')
        $modificaciones_mixtas = $modificaciones_mixtas->where('dee.tallos_x_ramos', '=', $tallos_x_ramos);
    if ($longitud_ramo != '')
        $modificaciones_mixtas = $modificaciones_mixtas->where('dee.longitud_ramo', '=', $longitud_ramo);
    if ($unidad_medida != '')
        $modificaciones_mixtas = $modificaciones_mixtas->where('dee.id_unidad_medida', '=', $unidad_medida);
    $modificaciones_mixtas = $modificaciones_mixtas->get();

    $val_mod = 0;
    foreach ($modificaciones_solidas as $mod) {
        $dee = $mod->detalle_especificacionempaque;
        $ramos_x_caja = $mod->ramos_x_caja != '' ? $mod->ramos_x_caja : $dee->cantidad;
        if ($mod->operador == '+') {
            $val_mod += $mod->cantidad * $ramos_x_caja;
            $actual -= $mod->cantidad * $ramos_x_caja;
        } else {
            $val_mod -= $mod->cantidad * $ramos_x_caja;
            $actual += $mod->cantidad * $ramos_x_caja;
        }
...
function getCantidadRamosPedidosDistribucionForCB(
    $fecha,
    $variedad,
    $presentacion,
    $tallos_x_ramos,
    ...
    $ramos = 0;
    foreach ($query as $q) {
        $ramos += $q->ramos * $q->piezas;
    }
    return $ramos;
}

function getCantidadRamosPedidosDistribucionForCBByMarcacionRangoFecha(
    $desde,
    $hasta,
    $variedad,
    $presentacion,
    ...
    $query = DB::table('distribucion_mixtos as m')
        ->join('detalle_especificacionempaque as dee', 'dee.id_detalle_especificacionempaque', '=', 'm.id_detalle_especificacionempaque')
        ->join('detalle_pedido as dp', 'dp.id_pedido', '=', 'm.id_pedido')
        ->join('pedido as p', 'p.id_pedido', '=', 'm.id_pedido')
        ->join('detallepedido_datoexportacion as dm', 'dm.id_detalle_pedido', '=', 'm.id_detalle_pedido')
        ->select(
            'm.*',
        )->distinct()
        ->where('m.id_planta', $variedad->id_planta)
        ...
        ->where('dm.id_dato_exportacion', '=', $id_dato_exportacion)
        ->where('dm.valor', '=', $marcacion)
        ->get();
    $ramos = 0;
    foreach ($query as $q) {
        $ramos += $q->ramos * $q->piezas;
...
function getCantidadRamosPedidosByMarcacion(
    $fecha,
    $variedad,
...
function getDistribucionByClienteLongitudFechaPlanta($cliente, $longitud, $fecha, $planta)
{
    $r = DB::table('distribucion_mixtos')
        ->select(
...
function getFlorDistribuidaByClienteLongitudFechaPlanta($cliente, $longitud, $fecha, $planta)
{
    $r = DB::table('mixtos as m')
        ->join('proyecto as p', 'p.id_proyecto', '=', 'm.id_proyecto')
        ->select(
            DB::raw('sum(m.tallos) as tallos'),
            DB::raw('sum(m.ramos * m.piezas) as ramos')
        )
        ->where('m.id_cliente', $cliente)
        ->where('m.id_planta', $planta)
        ->where('m.longitud_ramo', $longitud)
        ->where('p.fecha', $fecha)
        ->get()[0];
    return $r;
}

function getMixtosByClienteLongitudFechaPlanta($cliente, $longitud, $fecha, $planta)
{
    $query = DB::table('pedido as p')
        ->join('detalle_pedido as dp', 'dp.id_pedido', '=', 'p.id_pedido')
        ->join('cliente_pedido_especificacion as cpe', 'cpe.id_cliente_pedido_especificacion', '=', 'dp.id_cliente_especificacion')
        ...
        ->where('dee.longitud_ramo', $longitud)
        ->whereNotNull('dee.tallos_x_ramos')
        ->groupBy(
            'dp.id_detalle_pedido',
            'dp.cantidad',
...
function getFlorMixtosByClienteLongitudFechaPlanta($cliente, $longitud, $fecha, $planta)
{
    $query = DB::table('proyecto as p')
        ...
        ->where('v.assorted', 1)
        ->where('v.id_planta', $planta)
        ->where('dc.longitud_ramo', $longitud)
        ->whereNotNull('dc.tallos_x_ramo')
        ->get();
    return [
        'tallos' => $query[0]->tallos,
        'ramos' => $query[0]->ramos,
    ];
}
...
{
    return Color::where('nombre', $nombre)
        ->get()
        ->first();
}
...
function getColores()
{
    return Color::where('estado', 1)->orderBy('nombre')->get();
}
            ...
            if ($var->id_variedad == $variedad || $variedad == 'T') {
                ...
                foreach ($query as $q) {
                    ...
                    foreach ($semanas as $sem) {
                        if (
                            ($ciclo->fecha_fin >= $sem->fecha_inicial && $ciclo->fecha_fin <= $sem->fecha_final) ||
...
function getAreaActivaFromData($variedades, $semanas)
{
    ...
    foreach ($variedades as $pos_var => $variedad) {
        ...
        foreach ($semanas as $pos_sem => $semana)
            $total_variedad[] = 0;
        foreach ($variedad['ciclos'] as $ciclo) foreach ($ciclo['areas'] as $pos_area => $area)
...
function getCalibreMonitoreoByRangoVariedad($desde, $hasta, $variedad)
{
    $monitoreos = DB::table('monitoreo_calibre as m')
        ->join('ciclo as c', 'c.id_ciclo', '=', 'm.id_ciclo')
        ->select('m.*')->distinct()
        ->where('m.fecha', '>=', $desde)
        ->where('m.fecha', '<=', $hasta)
        ->where('c.estado', 1);
    if ($variedad != 'T') // x variedad
        $monitoreos = $monitoreos->where('c.id_variedad', $variedad);
    $monitoreos = $monitoreos->get();
...
{
    return Semana::where('estado', 1)->where('codigo', $codigo)->get()->first();
}
...
function getColorByIndicadorValor($nombre, $valor)
{
    $indicador = getIndicadorByName($nombre);
    $rangos = getIntervalosIndicador($nombre);
    if (count($rangos) > 0) {
...
function getRamosXCajaModificadoPerdido($idDetPed, $idDetEspEmp)
{
    return DetalleEspecificacionEmpaqueRamosXCajaPerdido::where([
        ['id_detalle_pedido_perdido', $idDetPed],
        ['id_detalle_especificacionempaque', $idDetEspEmp]
    ])->orderBy('id_detalle_especificacionempaque_ramos_x_caja_perdido', 'desc')->select('cantidad')->first();
}
...
{
    return DB::select('SELECT DATE_FORMAT(NOW( ), "%H:%i:%S" ) as hora;')[0]->hora;
}
...
function getColumnasExcel()
{
    ...
    $columnas = $default;
    foreach ($default as $x) foreach ($default as $y)
        $columnas[] = $x . $y;
...
function setBoldToCeldaExcel($sheet, $celda)
{
    $sheet->getStyle($celda)->getFont()->setBold(1);
}
...
function es_super_administrador()
{
    if (getUsuario(Session::get('id_usuario'))->id_rol == 1)
        return true;
    else
        return false;
}
...
function es_local()
{
    return env('ENTORNO') == 'LOCAL';
}

function es_server()
{
    return env('ENTORNO') == 'SERVER';
}

function ordenVariedades($planta = [])
{
    if (in_array(2, $planta) || in_array(3, $planta)) {
...
function isPedidoDistribuido($id_pedido)
{
    $query = DB::table('pedido as p')
        ->join('detalle_pedido as dp', 'dp.id_pedido', '=', 'p.id_pedido')
        ->join('cliente_pedido_especificacion as cpe', 'cpe.id_cliente_pedido_especificacion', '=', 'dp.id_cliente_especificacion')
    ...
    if ($tallos > 0) {
        $distribuidos = DB::table('distribucion_mixtos as m')
            ->join('detalle_pedido as dp', 'dp.id_detalle_pedido', '=', 'm.id_detalle_pedido')
            ->select(
                DB::raw('sum(m.tallos) as tallos'),
                DB::raw('sum(m.ramos) as ramos')
            )
            ->where('dp.id_pedido', $id_pedido)
            ->get()[0];
        //dump($distribuidos->tallos, $tallos);
        return $distribuidos->tallos == $tallos ? true : false;
    } else {
        return true;
    }
}
...
function getQueueForUpdateOrdenFija($pedido)
{
    $query = DB::table('jobs')
        ->select('queue')->distinct()
        ->where('queue', 'like', 'update_orden_fija%')
        ->where('payload', 'like', '%jobUpdateOrdenFija%pedido%";s:5:%"' . $pedido . '%detalle_pedido%fecha%eliminar_detalles%')
    ...
    $menor = [
        'cant' => '',
        'queue' => 'update_orden_fija_1',
    ];
    for ($i = 1; $i <= 10; $i++) {
        $j = Job::where('queue', 'update_orden_fija_' . $i)
            ->get();
        $count = count($j);
        if ($count == 0) {
            return 'update_orden_fija_' . $i;
        } else {
            if ($menor['cant'] == '' || $menor['cant'] > $count) {
                $menor['cant'] = $count;
                $menor['queue'] = 'update_orden_fija_' . $i;
            }
        }
    }
    return $menor['queue'];
...
function getPresentacionesByClientePlanta($cliente, $planta, $empaque)
{
    ...
    foreach ($presentaciones as $item) {
        $selected = $item->id_empaque_p == $empaque ? 'selected' : '';
        $option_presentaciones .= '<option value="' . $item->id_empaque_p . '" ' . $selected . '>' . $item->nombre . '</option>';
    }
    return $option_presentaciones;
}

function getLongitudesByClientePlanta($cliente, $planta, $longitud_selected)
{
    ...
    foreach ($longitudes as $item) {
        $selected = $item->longitud_ramo == $longitud_selected ? 'selected' : '';
        $option_longitudes .= '<option value="' . $item->longitud_ramo . '" ' . $selected . '>' . $item->longitud_ramo . 'cm</option>';
    }
    return $option_longitudes;
}

function getSobranteByVariedadLongitudFecha($var, $long, $fecha)
{
    return SobranteRecepcion::All()
...
function getInventarioFrioByMarcacion($variedad, $dato_exp, $marcacion, $empaque, $tallos_x_ramos, $longitud)
{
    return DB::table('inventario_frio')
        ->select(DB::raw('sum(disponibles) as cantidad'))
        ->where('id_variedad', $variedad)
        ->where('id_dato_exportacion', $dato_exp)
        ->where('valor_marcacion', $marcacion)
        ->where('id_empaque_p', $empaque)
        ->where('tallos_x_ramo', $tallos_x_ramos)
        ->where('longitud_ramo', $longitud)
        ->where('disponibilidad', 1)
        ->where('basura', 0)
        ->where('disponibles', '>', 0)
...
function getCuartoFrioByMarcacion($variedad, $dato_exp, $marcacion, $empaque, $tallos_x_ramos, $longitud)
{
    return DB::table('cuarto_frio')
        ->select(DB::raw('sum(disponibles) as cantidad'))
        ->where('id_variedad', $variedad)
        ->where('id_dato_exportacion', $dato_exp)
        ->where('valor_marcacion', $marcacion)
