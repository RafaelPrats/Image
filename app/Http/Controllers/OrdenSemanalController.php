...
class OrdenSemanalController extends Controller
{
    public function store_orden_semanal(Request $request)
    {
        ...
        if ($request->nueva_esp != '') {    // NUEVA ESPECIFICACION
            ...
            $esp->id_especificacion = isset($esp->id_especificacion) ? $esp->id_especificacion + 1 : 1;
            $esp->tipo = 'O';
            ...
            if ($esp->save()) {
                ...
                if ($esp_emp->save()) {
                    ...
                    $det_espemp->id_detalle_especificacionempaque = isset($detEspEmpaque->id_detalle_especificacionempaque) ? $detEspEmpaque->id_detalle_especificacionempaque + 1 : 1;
                    ...
                    if ($det_espemp->save()) {
                        ...
                        $cli_ped_esp->id_cliente_pedido_especificacion = isset($cpe->id_cliente_pedido_especificacion) ? $cpe->id_cliente_pedido_especificacion + 1 : 1;
                        ...
                        if ($cli_ped_esp->save()) {
                            ...
                            $pedido->id_pedido = isset($p->id_pedido) ? $p->id_pedido + 1 : 1;
                            ...
                            if ($pedido->save()) {
                                ...
                                $det_pedido->id_detalle_pedido = isset($detPed->id_detalle_pedido) ? $detPed->id_detalle_pedido + 1 : 1;
            ...
            if (count($request->arreglo_esp) > 0) {
                ...
                foreach ($request->arreglo_esp as $pos_esp => $esp) {
                    ...
                    if ($det_pedido->save()) {
                        ...
                        foreach ($arreglo_det_pedidos as $item){
                            DetallePedido::destroy($item->id_detalle_pedido);
                            //$item->delete();
                        }
                ...
                if ($pedido->save()) {
                    ...
                    /* =========== ENVIO ========= */
                    $envio = new Envio();
                    $e = Envio::orderBy('id_envio','desc')->first();
                    $envio->id_envio = isset($e->id_envio) ? $e->id_envio + 1 : 1;
                    ...
                    if ($envio->save()) {
                        ...
                        foreach ($pedido->detalles as $det) {
                            ...
                            $det_envio->id_detalle_envio = isset($detEnv->id_detalle_envio) ? $detEnv->id_detalle_envio + 1 : 1;
    ...
    public function update_orden_tinturada(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            if ($pedido->save()) {
                            ...
                            //foreach ($request->det_ped_arreglo_dat_exp as $dat_exp) {
                                foreach($request->det_ped_arreglo_dat_exp[$z] as $de){
                                    if ($de['valor'] != '') {
                                        ...
                                        $det_datexp->id_detallepedido_datoexportacion = isset($dpe->id_detallepedido_datoexportacion) ? $dpe->id_detallepedido_datoexportacion + 1 : 1;
                ...
                $envio->id_envio = isset($env->id_envio) ? $env->id_envio + 1 : 1;
                $envio->id_pedido = $pedido->id_pedido;
                $envio->fecha_envio = $request->fecha_envio;
    ...
    public function store_pedido_personalizado(Request $request)
    {
        ...
        if (count($request->arreglo) > 0) {
            foreach ($request->arreglo as $item) {
                ...
                $esp = Especificacion::orderBy('id_especificacion', 'desc')->first();
                $especificacion->id_especificacion = isset($esp->id_especificacion) ? $esp->id_especificacion + 1 : 1;
                ...
                if ($especificacion->save()) {
                    ...
                    if ($esp_emp->save()) {
                        ...
                        $detEspEmpaque = DetalleEspecificacionEmpaque::orderBy('id_detalle_especificacionempaque', 'desc')->first();
                        $det_esp_emp->id_detalle_especificacionempaque = isset($detEspEmpaque->id_detalle_especificacionempaque) ? $detEspEmpaque->id_detalle_especificacionempaque + 1 : 1;
