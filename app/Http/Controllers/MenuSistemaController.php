...
class MenuSistemaController extends Controller
{
    ...
    public function store_grupo_menu(Request $request)
    {
        ...
        if (!$valida->fails()) {
            $model = new GrupoMenu();
            $grupoMenu = GrupoMenu::orderBy('id_grupo_menu','desc')->first();
            $model->id_grupo_menu = isset($grupoMenu->id_grupo_menu) ? $grupoMenu->id_grupo_menu + 1 : 1;
    ...
    public function store_menu(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (count(Menu::All()->where('nombre', '=', espacios($request->nombre))
                ...
                $model = new Menu();
                $menu = Menu::orderBy('id_menu','desc')->first();
                $model->id_menu = isset($menu->id_menu) ? $menu->id_menu + 1 : 1;
    ...
    public function store_submenu(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (count(Submenu::All()->where('nombre', '=', espacios($request->nombre))
                ...
                $model = new Submenu();
                $subMenu = Submenu::orderBy('id_submenu','desc')->first();
                $model->id_submenu = isset($subMenu->id_submenu) ? $subMenu->id_submenu + 1 : 1;
