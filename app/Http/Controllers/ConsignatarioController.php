...
class ConsignatarioController extends Controller
{
    ...
    public function buscar_listado(Request $request){
        ...
        if ($request->busqueda != '') $listado = $listado->Where(function ($q) use ($busqueda) {
            $q->Where(DB::raw("UPPER(consignatario.nombre)"), 'like', '%' . mb_strtoupper($busqueda) . '%');
        });

        $listado = $listado->orderBy('consignatario.nombre', 'asc')->paginate(100);
    ...
    public function storeConsignatario(Request $request){
        ...
        if(!$valida->fails()) {
            ...
            if(!isset($request->id_consignatario)){
                ...
                $c = Consignatario::orderBy('id_consignatario','desc')->first();
                $objConsignatario->id_consignatario = isset($c->id_consignatario) ? $c->id_consignatario + 1 : 1;
                $letra = "I";
                $accion = "Inserción";
            }else{
            ...
            if(isset($request->cod_pais_jire))
                $objConsignatario->cod_pais_jire = $request->cod_pais_jire;
            ...
            if($objConsignatario->save()) {
                ...
                if($request->contacto == "true"){
                    if(empty($request->id_contacto_consignatario)){
                        ...
                        $cc = ContactoConsignatario::orderBy('id_contacto_consignatario','desc')->first();
                        $objContatoConsignatario->id_contacto_consignatario = isset($cc->id_contacto_consignatario) ? $cc->id_contacto_consignatario + 1 : 1;
                        $letra = "I";
                        $accion = "Inserción";
                    }else{
    ...
    public function storeContactoConsignatario(Request $request)
    {
        $valida = Validator::make($request->all(), [
            'id_consignatario'    => 'required',
        ...
        if(!$valida->fails()) {
            ...
            try{
                ...
                $cc = ContactoConsignatario::orderBy('id_contacto_consignatario','desc')->first();
                $objContatoConsignatario->id_contacto_consignatario = isset($cc->id_contacto_consignatario) ? $cc->id_contacto_consignatario + 1 : 1;
                $objContatoConsignatario->id_consignatario = $request->id_consignatario;
                $objContatoConsignatario->nombre = $request->nombre_contacto;
                $objContatoConsignatario->telefono = $request->telefono_contacto;
                $objContatoConsignatario->ciudad = $request->ciudad_contacto;
                $objContatoConsignatario->correo = $request->correo_contacto;
                ...
                if($objContatoConsignatario->save()){
                    $model = ContactoConsignatario::all()->last();
                    bitacora('contacto_consignatario', $model->id_contacto_consignatario, 'I', 'Inserción satisfactoria del contacto para el consignatario '.$request->nombre);
                }

                DB::commit();
                $success = true;
                $msg=   '<div class="alert alert-success text-center">' .
                            '<p> Se han guardado los contactos del ciente con éxito </p>'
                        .'</div>';
