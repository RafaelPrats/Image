...
class PlantaController extends Controller
{
    public function inicio(Request $request)
    {
        return view('adminlte.gestion.plantas_variedades.inicio', [
            ...
            'plantas' => Planta::orderBy('orden')->orderBy('estado', 'desc')->get(),
        ]);
    }
    ...
    public function store_planta(Request $request)
    {
        ...
        if (!$valida->fails()) {
            $model = new Planta();
            $planta = Planta::orderBy('id_planta', 'desc')->first();
            $model->id_planta = isset($planta->id_planta) ? $planta->id_planta + 1 : 1;
    ...
    public function add_variedad(Request $request)
    {
        return view('adminlte.gestion.plantas_variedades.forms.add_variedad', [
            ...
            'variedades' => Variedad::select('nombre', 'siglas')->distinct()->get()
        ]);
    }
    ...
    public function store_variedad(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $model = new Variedad();
            $variedad = Variedad::orderBy('id_variedad', 'desc')->first();
            $model->id_variedad = isset($variedad->id_variedad) ? $variedad->id_variedad + 1 : 1;
            $model->nombre = str_limit(mb_strtoupper(espacios($request->nombre)), 250);
            ...
            $model->orden = $request->orden;
            $model->receta = $request->receta == true ? 1 : 0;
    ...
    public function update_variedad(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (count(Variedad::All()->where('nombre', '=', str_limit(mb_strtoupper(espacios($request->nombre)), 250))
                ...
                $model->tallos_x_malla = $request->tallos_x_malla;
                $model->saldo_inicial = $request->saldo_inicial;
                $model->desecho_propag_disponibilidad = $request->desecho_propag_disponibilidad;
                $model->cant_semanas_propag = $request->cant_semanas_propag;
                $model->orden = $request->orden;
                $model->receta = $request->receta == 'true' ? 1 : 0;
        ...
        if (!$valida->fails()) {
            ...
            foreach ($request->arrData as $key => $precio) {
                ...
                if (count($existPrecio) > 0) {
                    ...
                    $p = Precio::orderBy('id_precio', 'desc')->first();
                    $precio->id_precio = isset($p->id_precio) ? $p->id_precio + 1 : 1;
                    $precio = Precio::orderBy('id_precio', 'desc')->first();
                    $objPrecio->id_precio = isset($precio->id_precio) ? $precio->id_precio + 1 : 1;
                    $palabra = 'Insertado';
                    $accion = 'I';
                }
    ...
    public function store_vinculo(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            if ($model == '') {
                ...
                $vcu =  VariedadClasificacionUnitaria::orderBy('id_variedad_clasificacion_unitaria', 'desc')->first();
                $model->id_variedad_clasificacion_unitaria = isset($vcu->id_variedad_clasificacion_unitaria) ? $vcu->id_variedad_clasificacion_unitaria + 1 : 1;
    ...
    public function add_regalias(Request $request)
    {
        ...
        return view('adminlte.gestion.plantas_variedades.forms.add_regalias', [
            'variedad' => $variedad,
            'regalias' => isset($semana_actual) ? $variedad->regaliasBySemana($semana_actual->codigo) : null,
    ...
    public function store_regalias(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            if ($model == '') {
                $model = new Regalias();
                $regalias = Regalias::orderBy('id_regalias', 'desc')->first();
                $model->id_regalias = isset($regalias->id_regalias) ? $regalias->id_regalias + 1 : 1;
    ...
    public function distribuciones(Request $request)
    {
        ...
        $longitudes = DB::table('distribucion_variedad')
            ->select('longitud', 'id_unidad_medida')->distinct()
        ...
        foreach ($variedades as $var) {
            ...
            foreach ($longitudes as $l) {
                $valor = DistribucionVariedad::All()
                    ->where('id_planta', $planta->id_planta)
                    ->where('siglas', $var->siglas)
                    ->where('longitud', $l->longitud)
                    ->where('id_unidad_medida', $l->id_unidad_medida)
                    ->first();
        ...
        return view('adminlte.gestion.plantas_variedades.forms.distribuciones', [
            'planta' => $planta,
            'listado' => $listado,
    ...
    {
        $distribuciones = DistribucionVariedad::where('id_planta', $request->planta)->get();

        foreach ($distribuciones as $d)
            DistribucionVariedad::destroy($d->id_distribucion_variedad);

        if (isset($request->data))
            foreach ($request->data as $d) {
                $model = new DistribucionVariedad();
                $model->id_planta = $request->planta;
                $model->siglas = $d['var'];
                $model->longitud = $d['longitud'];
                $model->valor = $d['valor'];
                $model->id_unidad_medida = $d['unidad'];
                $model->save();
            }
        return [
            'success' => true,
            'mensaje' => 'Se ha <strong>GUARDADO</strong> la distribución correctamente',
        ];
    }
    ...
    public function admin_receta(Request $request)
    {
        ...
        return view('adminlte.gestion.plantas_variedades.forms.admin_receta', [
            'variedad' => $variedad,
            'plantas' => Planta::All()->where('estado', '=', 1),
        ]);
    }
    ...
    public function buscar_variedades(Request $request)
    {
        ...
        return view('adminlte.gestion.plantas_variedades.forms.buscar_variedades', [
            'listado' => $listado,
        ]);
    }
    ...
    public function store_agregar_variedades(Request $request)
    {
        ...
        foreach (json_decode($request->data) as $d) {
            ...
            $model->id_variedad = $request->id_var;
        ...
        return [
            'success' => true,
            'mensaje' => 'Se han <strong>ASIGNADO</strong> los tallos correctamente',
        ];
    }
}
