...
class NotificacionesController extends Controller
{
    ...
    public function store_notificacion(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $notificacion = Notificacion::orderBy('id_notificacion', 'desc')->first();
            $model->id_notificacion = isset($notificacion->id_notificacion) ? $notificacion->id_notificacion + 1 : 1;
    ...
    public function save_notificacion_usuario(Request $request)
    {
        ...
        if ($model == '') {
            ...
            $notificacionUsuario = NotificacionUsuario::orderBy('id_notificacion_usuario', 'desc')->first();
            $model->id_notificacion_usuario = isset($notificacionUsuario->id_notificacion_usuario) ? $notificacionUsuario->id_notificacion_usuario + 1 : 1;
    ...
    {
        $news = UserNotification::join('notificacion as n', 'n.id_notificacion', '=', 'user_notification.id_notificacion')
            ->select('user_notification.*')->distinct()
            ->where('user_notification.estado', '=', 1)
            ->where('user_notification.id_usuario', '=', Session::get('id_usuario'))
            ->where('n.tipo', '=', $request->tipo)
            ->orderBy('user_notification.id_user_notification', 'desc')
            ->get();
        $li = '';
        if (count($news) > 0) {
            foreach ($news as $n) {
                $url = "'" . $n->url . "'";
                $n->texto = str_replace("'", "", $n->texto);
                $n->texto = str_replace('"', '', $n->texto);
                $n->texto = "'" . $n->texto . "'";
                $notificacion = $n->notificacion;
                if ($notificacion->automatica == 1 && $n->url != null) {
                    $li .= '<li title="' . $n->titulo . '">
                            <div class="row" style="padding: 5px">
                            <div class="col-md-12">
                            <a href="javascript:void(0)" class="text-color_yura" onclick="cargar_url(' . $url . ')">
                            <i class="fa fa-' . $notificacion->icono->nombre . '"></i> ' . $n->texto . '
                            </a>
                            </div>
                            </div>
                            </li>';
                    ...
                    $li .= '<li title="' . $n->titulo . '" id="li_user_notification_' . $n->id_user_notification . '">
                            ...
                            <i class="fa fa-' . $notificacion->icono->nombre . '"></i> ' . $n->titulo . '
                            </a>
                            </div>
