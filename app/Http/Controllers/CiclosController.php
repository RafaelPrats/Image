...
class CiclosController extends Controller
{
    ...
    public function ver_cosechas(Request $request)
    {
        ...
        $cosechas = DB::table('desglose_recepcion as dr')
            ...
            ->select('r.id_cosecha as id','r.fecha_ingreso')->distinct()
            ->where('dr.estado', '=', 1)
            ->where('r.estado', '=', 1)
    ...
    public function store_ciclo(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $ciclo = new Ciclo();
            $c = Ciclo::orderBy('id_ciclo', 'desc')->first();
            $ciclo->id_ciclo = isset($c->id_ciclo) ? $c->id_ciclo + 1 : 1;
            ...
            $factor_conteo = $request->tipo_planta == 'S' ? 1 : 2;
            $ciclo->tipo_planta = $request->tipo_planta;
            $ciclo->plantas_muertas = $request->plantas_muertas > 0 ? round($request->plantas_muertas * $factor_conteo, 2) : 0;
            ...
            if ($ciclo->poda_siembra == 'P') {
                ...
                $ciclo->conteo = $request->conteo > 0 ? round($request->conteo / $factor_conteo, 2) : $semana->tallos_planta_poda;
            } else {
                $ciclo->semana_poda_siembra = $semana->semana_siembra;
                $ciclo->conteo = $request->conteo > 0 ? round($request->conteo / $factor_conteo, 2) : $semana->tallos_planta_siembra;
            }
            ...
            $ciclo->ancho_cama = $getLastCiclo->ancho_cama;
            $ciclo->ancho_camino = $getLastCiclo->ancho_camino;
            ...
            if ($ciclo->save()) {
                ...
                foreach ($proyecciones as $proy) {
                    $p = ProyeccionModulo::find($proy->id_proyeccion_modulo);
                    $p->estado = 0;
                ...
                $pm = ProyeccionModulo::orderBy('id_proyeccion_modulo','desc')->first();
                $proy->id_proyeccion_modulo = isset($pm->id_proyeccion_modulo) ? $pm->id_proyeccion_modulo + 1 : 1;
    ...
    public function update_ciclo(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            if (getSemanaByDate($model->fecha_inicio)->codigo != getSemanaByDate($request->fecha_inicio)->codigo) {   // hay que mover las proyecciones
                $cant_semanas_ciclo = $model->semana_poda_siembra + count(explode('-', $semana_req->curva)) - 1;
                $semana_new_req = getSemanaByDateVariedad(opDiasFecha('+', $cant_semanas_ciclo * 7, $request->fecha_inicio), $request->variedad);

                $proy = ProyeccionModulo::All()->where('estado', 1)
            ...
            $ciclo->fecha_fin = $request->fecha_fin;
            $ciclo->plantas_iniciales = $request->plantas_iniciales;
            if ($request->has('tipo_planta'))
                $ciclo->tipo_planta = $request->tipo_planta;
            $factor_conteo = $request->tipo_planta == 'S' ? 1 : 2;
            if ($ciclo->plantas_muertas != $request->plantas_muertas)
                $ciclo->plantas_muertas = $request->plantas_muertas > 0 ? round($request->plantas_muertas * $factor_conteo, 2) : 0;
            ...
            //$ciclo->desecho = $semana->desecho != '' ? $semana->desecho : 0;
            //$ciclo->curva = $semana->curva;
            if ($ciclo->conteo != $request->conteo)
                if ($ciclo->poda_siembra == 'P') {
                    //$ciclo->semana_poda_siembra = $semana->semana_poda;
                    $ciclo->conteo = $request->conteo > 0 ? round($request->conteo / $factor_conteo, 2) : $semana->tallos_planta_poda;
                } else {
    ...
    public function terminar_ciclo(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $ciclo =  Ciclo::find(isset($ciclo) ? $ciclo->id_ciclo : 0);
    ...
    public function store_nuevo_ciclo(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $ciclo->tipo_planta = $request->tipo_planta;
            $factor_conteo = $request->tipo_planta == 'S' ? 1 : 2;
            $ciclo->conteo = $request->conteo > 0 ? round($request->conteo / $factor_conteo, 2) : '';
            $ciclo->plantas_iniciales = $request->plantas_iniciales;
            $ciclo->plantas_muertas = $request->plantas_muertas > 0 ? round($request->plantas_muertas * $factor_conteo, 2) : 0;
    ...
    public function update_ciclo_campo(Request $request)
    {
        $model = Ciclo::find($request->id);
        $model->ancho_cama = $request->ancho_cama > 0 ? $request->ancho_cama : 0;
        $model->ancho_camino = $request->ancho_camino > 0 ? $request->ancho_camino : 0;
        $model->save();
        return [
            'success' => true,
            'mensaje' => 'Se ha <strong>actualizado</strong> el ciclo satisfactoriamente',
        ];
    }

    public function update_all_ciclo_campo(Request $request)
    {
        foreach ($request->data as $data) {
            $model = Ciclo::find($data['id']);
            $model->ancho_cama = $data['ancho_cama'] > 0 ? $data['ancho_cama'] : 0;
            $model->ancho_camino = $data['ancho_camino'] > 0 ? $data['ancho_camino'] : 0;
            $model->save();
        }
        return [
            'success' => true,
            'mensaje' => 'Se han <strong>actualizado</strong> los ciclos satisfactoriamente',
        ];
    }
}
