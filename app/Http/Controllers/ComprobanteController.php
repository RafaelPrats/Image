...
class ComprobanteController extends Controller
{
    ...
    public function buscar_comprobante(Request $request)
    {
        ...
        $listado = $listado->orderBy('secuencial', 'Desc')
    ...
    public function generar_comprobante_factura(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            try{
                ...
                $cliente = $envio->pedido->cliente->detalle()->nombre;
                ...
                if ($request->update == "true") {
                    ...
                    $dataComprobante = $request->id_comprobante == null
                        ?   Comprobante::where('id_envio', $request->id_envio)
                        :   Comprobante::where('comprobante.id_comprobante', $request->id_comprobante);
                    ...
                    $id_pedido = $request->id_comprobante == null
                        ?  getEnvio($request->id_envio)->pedido->id_pedido
                        :  Pedido::where('id_comprobante_temporal', $request->id_comprobante)->first()->id_pedido;

                    $objPedido = Pedido::find($id_pedido);
                    $objPedido->clave_acceso_temporal = null;
                    $objPedido->id_comprobante_temporal = null;
                    $objPedido->save();
                    Comprobante::destroy($dataComprobante[0]->id_comprobante);

                } else {

                    $seq = Comprobante::orderBy('id_comprobante', 'desc')->first();

                    $secuencial = isset($envio->pedido->clave_acceso_temporal)
                    ?  $envio->pedido->clave_acceso_temporal
                    :  ( isset($seq->secuencial) ? ( ( str_pad((int)$seq->secuencial+1 , 15,'0',STR_PAD_LEFT) ) ) : '000000000000001') ;
                }
                ...
                $comp = Comprobante::orderBy('id_comprobante','desc')->first();
                $obj_comprobante->id_comprobante = isset($comp->id_comprobante) ? $comp->id_comprobante+1: 1;
                ...
                $obj_comprobante->ambiente = 2;//env('ENTORNO');
                ...
                if ($obj_comprobante->save()) {
                    ...
                    $detFact = DetalleFactura::orderBy('id_detalle_factura','desc')->first();
                    $objDetalleFactura->id_detalle_factura = isset($detFact->id_detalle_factura) ? $detFact->id_detalle_factura+1 :1;
                    ...
                    if ($objDetalleFactura->save()) {
                        ...
                        $impDetFact = ImpuestoDetalleFactura::orderBy('id_impuesto_detalle_factura','desc')->first();
                        $objImpuestoDetalleFactura->id_impuesto_detalle_factura = isset($impDetFact->id_impuesto_detalle_factura) ? $impDetFact->id_impuesto_detalle_factura+1 :1;
                        ...
                        if ($objImpuestoDetalleFactura->save()) {
                                ...
                                foreach ($envio->pedido->detalles as $x => $det_ped) {
                                    ...
                                    foreach ($det_ped->cliente_especificacion->especificacion->especificacionesEmpaque as $m => $esp_emp) {
                                        foreach ($esp_emp->detalles as $n => $det_esp_emp) {
                                            ...
                                            $desGEnvFact = DesgloseEnvioFactura::orderBy('id_desglose_envio_factura','desc')->first();
                                            $objDesgloseEnvioFactura->id_desglose_envio_factura = isset($desGEnvFact->id_desglose_envio_factura) ? $desGEnvFact->id_desglose_envio_factura+1 :1;
                                            ...
                                            if ($objDesgloseEnvioFactura->save()) {
                                                ...
                                                $impDesgEnvFact = ImpuestoDesgloseEnvioFactura::orderBy('id_impuesto_desglose_envio_factura','desc')->first();
                                                $objImpuestoDesgloseEnvioFactura->id_impuesto_desglose_envio_factura = isset($impDesgEnvFact->id_impuesto_desglose_envio_factura) ? $impDesgEnvFact->id_impuesto_desglose_envio_factura+1 :1;
                            ...
                            $msg .= "<div class='alert text-center  alert-success'>" .
                                    "<p>Se ha registrado con éxito la venta para " . $cliente . "</p>"
                                    . "</div>";
                ...
                $msg .= "<div class='alert text-center  alert-danger'>" .
                        "<p>Hubo un error al guardar la venta para " . $cliente . " en la base de datos</p>
            ...
            if (!$valida->fails()) {
                if ($request->update === "true") {
                    ...
                    foreach ($dataComprobante as $item)
                        foreach(ImpuestoDesgloseEnvioFactura::where('id_desglose_envio_factura', $item->id_impuesto_desglose_envio_factura)->get() as $delImpDesgEnvFact)
                            ImpuestoDesgloseEnvioFactura::destroy($delImpDesgEnvFact->id_impuesto_desglose_envio_factura);

                    foreach(DesgloseEnvioFactura::where('id_comprobante', $dataComprobante[0]->id_comprobante)->get() as $delDesgEnvFact)
                        DesgloseEnvioFactura::destroy($delDesgEnvFact->id_desglose_envio_factura);

                    foreach(ImpuestoDetalleFactura::where('id_detalle_factura', $dataComprobante[0]->id_detalle_factura)->get() as $delImpDetFact)
                        ImpuestoDetalleFactura::destroy($delImpDetFact->id_impuesto_detalle_factura);

                    foreach(DetalleFactura::where('id_comprobante', $dataComprobante[0]->id_comprobante)->get() as $delDetFact)
                        DetalleFactura::destroy($delDetFact->id_detalle_factura);
                ...
                if ($obj_comprobante->save()) {
                    ...
                    $detGuiaRem = DetalleGuiaRemision::orderBy('id_detalle_guia_remision','desc')->first();
                    $objDetalleGuiaRemision->id_detalle_guia_remision = isset($detGuiaRem->id_detalle_guia_remision) ? $detGuiaRem->id_detalle_guia_remision+1: 1;
    ...
    public function generar_comprobante_guia_remision_factura(Request $request)
    { //PARA EL VENTURE
        $msg = "";
        $success = false;
        $despacho = getDetalleDespacho(getComprobante($request->id_comprobante)->envio->pedido->id_pedido);
