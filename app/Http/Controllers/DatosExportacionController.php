...
class DatosExportacionController extends Controller
{
    ...
    public function store_datos_exportacion(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            foreach ($request->arrDatosExportacion as $datosExportacion) {
                ...
                if (empty($datosExportacion['id_dato_exportacion'])) {

                    $objDatoExportacion = new DatosExportacion();
                    $expotacion = DatosExportacion::orderBy('id_dato_exportacion', 'desc')->first();
                    $objDatoExportacion->id_dato_exportacion = isset($expotacion->id_dato_exportacion) ? $expotacion->id_dato_exportacion + 1 : 1;
                } else {

                    $objDatoExportacion = DatosExportacion::find($datosExportacion['id_dato_exportacion']);
                }
    ...
    public function asignar_dato_exportacion(Request $request)
    {
        ...
        if ($request->check == "true") {
            ...
            $cde = ClienteDatoExportacion::orderBy('id_cliente_datoexportacion', 'desc')->first();
            $objClienteDatoExportacion->id_cliente_datoexportacion = isset($cde->id_cliente_datoexportacion) ? $cde->id_cliente_datoexportacion + 1 : 1;
            ...
            if (ClienteDatoExportacion::destroy($objClienteDatoExportacion->id_cliente_datoexportacion)) {
                $msg = '<div class="alert alert-success text-center">' .
