...
class EnvioController extends Controller
{
            ...
            foreach ($request->arrData as $key => $detalle_envio) {
                $existDataEnvio = DetalleEnvio::join('envio as e','detalle_envio.id_envio','=','e.id_envio')
                    ...
                    if($existDataEnvio == null){
                        ...
                        $objEnvio->id_envio = isset($e->id_envio) ? $e->id_envio + 1 : 1;
    ...
    public function actualizar_envio(Request $request)
    {
        $valida = Validator::make($request->all(), [
            ...
            'guia_madre' => 'nullable|size:11'
        ],[
            'email.required' => 'Debe colocar el email del cliente',
            'fecha_envio.required' => 'Debe colocar la fecha del pedido',
            //'guia_madre.required' => 'Debe colocar la guía madre',
            'guia_madre.size' => 'La guía madre debe ser 11 dígitos',
        ...
        if (!$valida->fails()) {
            ...
            if($objEnvio->save()){
                ...
                $save_det_env = DetalleEnvio::All()->where('id_envio',$request->id_envio)->first();
                $save_det_env->id_aerolinea =$request->aerolinea;
                $save_det_env->save();
