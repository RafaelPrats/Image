...
class Area
{
    ...
    {
        $model = getIndicadorByName('D7');  // Área en produccion (-7 días)
        if ($model != '') {
            $desde = getSemanaByDate(opDiasFecha('-', 28, date('Y-m-d')));
            $hasta = getSemanaByDate(opDiasFecha('-', 7, date('Y-m-d')));

            $area = 0;
            $data_4semanas = getAreaCiclosByRango($desde->codigo, $hasta->codigo, 'T');

            foreach ($data_4semanas['variedades'] as $pos_var => $var) {
                foreach ($var['ciclos'] as $pos_c => $c) {
                    foreach ($c['areas'] as $pos_a => $a) {
                        dump('D7 - var: ' . ($pos_var + 1) . '/' . count($data_4semanas['variedades']) .
                            ' - ciclo: ' . ($pos_c + 1) . '/' . count($var['ciclos']) .
                            ' - area: ' . ($pos_a + 1) . '/' . count($c['areas']));
                        $area += $a;
                    }
                }
            }
            ...
            foreach ($variedades as $pos_var => $var) {
                ...
                $area = 0;
                $data_4semanas = getAreaCiclosByRango($desde->codigo, $hasta->codigo, $var->id_variedad);
    ...
    public static function ramos_m2_anno_4_semanas_atras()
    {
        ...
        if ($model != '') {
            ...
            $data_ciclos = getCiclosCerradosByRango($desde->codigo, $hasta->codigo, 'T');
            $ciclo = $data_ciclos['ciclo'];
            $area_cerrada = $data_ciclos['area_cerrada'];
            $tallos_ciclo = $data_ciclos['tallos_cosechados'];

            $data_cosecha = getCosechaByRango($desde->codigo, $hasta->codigo, 'T');
            ...
            foreach ($variedades as $pos_var => $var) {
                ...
                $data_ciclos = getCiclosCerradosByRango($desde->codigo, $hasta->codigo, $var->id_variedad);
                $ciclo = $data_ciclos['ciclo'];
                $area_cerrada = $data_ciclos['area_cerrada'];
                $tallos_ciclo = $data_ciclos['tallos_cosechados'];
                ...
                $resumen = ResumenSemanaCosecha::All()
                    ...
                    ->where('codigo_semana', '<=', $hasta->codigo);
                $cant_calibres = 0;
