...
class Postcosecha
{
    public static function calibre_7_dias_atras()
    {
        ...
        if ($model != '') {
            ...
            foreach ($variedades as $pos_v => $var) {
                dump('D1 - var: ' . ($pos_v + 1) . '/' . count($variedades));
    ...
    public static function tallos_clasificados_7_dias_atras()
    {
        ...
        if ($model != '') {
            ...
            $valor = 0;
            foreach ($verdes as $pos_verd => $v) {
                dump('D2 - verde: ' . ($pos_verd + 1) . '/' . count($verdes));
            ...
            $variedades = Variedad::where('estado', 1)->get();
            foreach ($variedades as $pos_v => $var) {
                dump('D2 - var: ' . ($pos_v + 1) . '/' . count($variedades));
                $ind = IndicadorVariedad::All()
                    ->where('id_indicador', $model->id_indicador)
                    ->where('id_variedad', $var->id_variedad)
                    ->first();
    ...
    public static function cajas_cosechadas_7_dias_atras()
    {
        ...
        if ($model != '') {
            ...
            foreach ($verdes as $pos_v => $v) {
                dump('P1 - verde: ' . ($pos_v + 1) . '/' . count($verdes));
    ...
    public static function rendimiento_desecho_7_dias_atras()
    {
        ...
        if ($model_1 != '' && $model_2 != '') {
            ...
            foreach ($fechas as $pos_f => $f) {
                dump('D5 - D6 - fecha: ' . ($pos_f + 1) . '/' . count($fechas));
