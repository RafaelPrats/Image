...
class SemanaController extends Controller
{
    ...
    public function procesar(Request $request)
    {
        $msg = '';
        $success = true;
        if (count(Semana::where('anno', '=', $request->anno)
            ->where('id_variedad', '=', $request->id_variedad)->get()) == 0) {
            if ($request->fecha_inicial < $request->fecha_final) {
                ...
                if (count($arreglo) >= 52 && count($arreglo) <= 53) {
                    ...
                    for ($i = 0; $i < count($arreglo); $i++) {
                        $model = new Semana();
                        $semana = Semana::orderBy('id_semana', 'desc')->first();
                        $model->id_semana = isset($semana->id_semana) ? $semana->id_semana + 1 : 1;
                        ...
                        if ($model->save()) {
                            $model->id_semana = DB::table('semana')
                                ->select(DB::raw('max(id_semana) as id'))
                                ->get()[0]->id;
    ...
    public function update_semana(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $objSemana = Semana::where('codigo', $model->codigo)->get();
            foreach ($objSemana as $obj) {
                $s = Semana::find($obj->id_semana);
                $s->mes = $request->mes;
                $s->save();
            }
            ...
            foreach (getVariedades() as $var) {
                ...
                if (count($sem_var) == 0)
                    foreach ($semanas as $sem) {
                        $new = new Semana();
                        $semana = Semana::orderBy('id_semana', 'desc')->first();
                        $new->id_semana = isset($semana->id_semana) ? $semana->id_semana + 1 : 1;
    ...
    public function actualizar_proyecciones_by_semanas(Request $request)
    {
        foreach ($request->semanas as $id) {
            ...
            $semana_fin = getLastSemanaByVariedad($semana->id_variedad);
            $ciclos = Ciclo::where('fecha_inicio', '>=', $semana->fecha_inicial)
                ->where('fecha_inicio', '<=', $semana->fecha_final)
                ->where('id_variedad', $semana->id_variedad)
                ->where('estado', 1)
                ->where('activo', 1)
                ->where('no_recalcular_curva', 0)
                ->whereNull('fecha_cosecha')
                ->get();
            foreach ($ciclos as $c) {
                $ciclo = Ciclo::find($c->id_ciclo);
                $ciclo->conteo = $ciclo->poda_siembra == 'P' ? $semana->tallos_planta_poda : $semana->tallos_planta_siembra;
                $ciclo->save();
                $next_proy = ProyeccionModulo::All()->where('id_modulo', $ciclo->id_modulo)
                    ->where('id_variedad', $ciclo->id_variedad)
                    ->orderBy('fecha_inicio', 'desc')
                    ->first();
                $next_proy->tallos_planta = $semana->tallos_planta_poda;
                $next_proy->save();
                /* ======================== ACTUALIZAR LA TABLA PROYECCION_MODULO_SEMANA ====================== */
                $semana_ini = getSemanaByDate($c->fecha_inicio);
                ProyeccionUpdateSemanal::dispatch($semana_ini->codigo, $semana_fin->codigo, $c->id_variedad, $c->id_modulo, 0)
                    ->onQueue('proy_cosecha');
            }
        }
        return [
            'success' => true,
            'mensaje' => '<div class="alert alert-info text-center">Se estan actualizando las proyecciones en segundo plano</div>',
        ];
    }
}
