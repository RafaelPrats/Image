...
class PermisoController extends Controller
{
    ...
    public function store_rol(Request $request)
    {
        ...
        if (!$valida->fails()) {
            $model = new Rol();
            $rol = Rol::orderBy('id_rol','desc')->first();
            $model->id_rol = isset($rol->id_rol) ? $rol->id_rol + 1 : 1;
    ...
    public function store_submenu(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (count(Rol_Submenu::All()->where('id_rol', '=', $request->id_rol)
                ...
                $rolSubMenu = Rol_Submenu::orderBy('id_rol_submenu','desc')->first();
                $model->id_rol_submenu = isset($rolSubMenu->id_rol_submenu) ? $rolSubMenu->id_rol_submenu + 1 : 1;
