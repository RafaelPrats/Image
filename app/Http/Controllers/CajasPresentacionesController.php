...
use yura\Modelos\EmpaqueProducto;
...
class CajasPresentacionesController extends Controller
{
    ...
    public function buscar_empaque(Request $request)
    {
        ...
        return view('adminlte.gestion.caja_presentacion.partials.listado', [
            'empaques' => Empaque::select('nombre', 'id_empaque', 'tipo', 'estado')
                ->where('tipo', 'P')->orWhere('tipo', 'C')->orderBy('estado', 'desc')->orderBy('id_empaque', 'desc')->get()
        ]);
    }
    ...
    public function store_empaque(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (empty($request->id_empaque)) {
                ...
                if ($request->tipo == 'C')
                    $objEmpaque->nombre = $request->nombre . '|' . $request->conversion . '|' . $request->peso;
                else
            ...
            $objEmpaque->cod_jire =  $request->cod_jire;
            $msg = '';

            if ($objEmpaque->save()) {
                $model = Empaque::orderBy('id_empaque', 'desc')->first();
                bitacora('empaque', empty($request->id_empaque) ? $model->id_empaque : $request->id_empaque, $letra, $accion . ' satisfactoria de un empaque');
    ...
    public function admin_productos(Request $request)
    {
        $empaque = Empaque::find($request->id_empaque);
        $plantas = Planta::where('estado', 1)
            ->orderBy('orden')
            ->get();
        return view('adminlte.gestion.caja_presentacion.forms.admin_productos', [
            'empaque' => $empaque,
            'plantas' => $plantas,
        ]);
    }
    ...
    public function buscar_detalles(Request $request)
    {
        ...
        return view('adminlte.gestion.caja_presentacion.forms._detalles', [
            'empaque' => $empaque,
    ...
    public function buscar_productos(Request $request)
    {
        ...
        return view('adminlte.gestion.caja_presentacion.forms.buscar_productos', [
            'listado' => $listado,
        ]);
    }
    ...
    {
        $delete = EmpaqueProducto::All()
            ->where('id_empaque', $request->id_emp)
            ->where('id_planta', $request->planta);
        foreach ($delete as $del)
            $del->delete();

        foreach (json_decode($request->data) as $d) {
            $model = new EmpaqueProducto();
            $model->id_empaque = $request->id_emp;
            $model->id_planta = $request->planta;
            $model->id_producto = $d->id_prod;
        ...
        return [
            'success' => true,
            'mensaje' => 'Se han <strong>ASIGNADO</strong> los productos correctamente',
        ];
    }
}
