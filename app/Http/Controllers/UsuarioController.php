...
class UsuarioController extends Controller
{
    ...
    public function store_usuarios(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            $model->id_usuario = isset($ultimoUsuario) ? $ultimoUsuario->id_usuario+1 : 1;
            $model->estado = 'A';
    ...
    public function update_usuario(Request $request)
    {
        ...
        if (!$valida->fails()) {
            if (count(Usuario::All()
                ...
                if ($model->save()) {
                    $msg = 'Se ha <strong>guardado</strong> el usuario satisfactoriamente';
    ...
    public function asignar_sectores(Request $request)
    {
        return view('adminlte.gestion.usuarios.forms.asignar_sectores', [
            'sectores' => Sector::where('estado', 1)->where('interno', 1)->orderBy('nombre')->get(),
            'asignados' => UsuarioSector::where('id_usuario', $request->usuario)->get(),
            'usuario' => $request->usuario,
        ]);
    }

    public function store_usuario_sector(Request $request)
    {
        if ($request->estado == 'true') {   // crear
            $model = new UsuarioSector();
            $usuarioSector =  UsuarioSector::orderBy('id_usuario_sector', 'desc')->first();
            $model->id_usuario_sector = isset($usuarioSector) ? $usuarioSector->id_usuario_sector + 1 : 1;
            $model->id_usuario = $request->usuario;
            $model->id_sector = $request->sector;
            $model->save();
        } else {    // borrar
            $model = UsuarioSector::All()
