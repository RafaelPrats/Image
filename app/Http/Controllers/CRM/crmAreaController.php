...
class crmAreaController extends Controller
{
    ...
    public function exportar_regalias(Request $request)
    {
        $spread = new Spreadsheet();
        $this->excel_regalias($spread, $request);
        $spread->getProperties()
            ->setTitle('Regalias_Semanas');

        $fileName = "Regalias_Semanas.xlsx";
    ...
    public function excel_regalias($spread, $request)
    {
        $data = getAreaCiclosByRango($request->desde, $request->hasta, $request->variedad);
        $variedades = $data['variedades'];
        $semanas = $data['semanas'];
        ...
        foreach ($variedades as $pos_var => $variedad) {
            if ($pos_var == 0)
                $objSheet = $spread->getActiveSheet()->setTitle($variedad['variedad']->nombre);
            else
                $objSheet = $spread->createSheet()->setTitle($variedad['variedad']->nombre);
            $total_variedad = [];

            $row = 1;
            $col = 0;
            setValueToCeldaExcel($objSheet, $columnas[$col] . $row, 'Módulo');
            setBgToCeldaExcel($objSheet, $columnas[$col] . $row, '00b388');   // verde
            setColorTextToCeldaExcel($objSheet, $columnas[$col] . $row, 'FFFFFF');   // blanco
            $col++;
            setValueToCeldaExcel($objSheet, $columnas[$col] . $row, 'Fecha Inicio');
            setBgToCeldaExcel($objSheet, $columnas[$col] . $row, '00b388');   // verde
            setColorTextToCeldaExcel($objSheet, $columnas[$col] . $row, 'FFFFFF');   // blanco
            $col++;
            setValueToCeldaExcel($objSheet, $columnas[$col] . $row, 'Semana Inicio');
            setBgToCeldaExcel($objSheet, $columnas[$col] . $row, '00b388');   // verde
            setColorTextToCeldaExcel($objSheet, $columnas[$col] . $row, 'FFFFFF');   // blanco
            foreach ($semanas as $pos_sem => $semana) {
                $col++;
                setValueToCeldaExcel($objSheet, $columnas[$col] . $row, $semana->codigo);
                setBgToCeldaExcel($objSheet, $columnas[$col] . $row, 'e9ecef');   // gris
                $total_variedad[] = 0;
            }
            foreach ($variedad['ciclos'] as $ciclo) {
                $row++;
                $col = 0;
                setValueToCeldaExcel($objSheet, $columnas[$col] . $row, $ciclo['ciclo']->modulo->nombre);
                $col++;
                setValueToCeldaExcel($objSheet, $columnas[$col] . $row, $ciclo['ciclo']->fecha_inicio);
                $col++;
                setValueToCeldaExcel($objSheet, $columnas[$col] . $row, getSemanaByDate($ciclo['ciclo']->fecha_inicio)->codigo);
                foreach ($ciclo['areas'] as $pos_area => $area) {
                    $col++;
                    setValueToCeldaExcel($objSheet, $columnas[$col] . $row, round($area, 2));
                    $total_variedad[$pos_area] += $area;
                }
            }
            $row++;
            $col = 0;
            setValueToCeldaExcel($objSheet, $columnas[$col] . $row, 'TOTAL');
            setBgToCeldaExcel($objSheet, $columnas[$col] . $row, '00b388');   // verde
            setColorTextToCeldaExcel($objSheet, $columnas[$col] . $row, 'FFFFFF');   // blanco
            $objSheet->mergeCells($columnas[$col] . $row . ':' . $columnas[$col + 2] . $row);
            $col += 2;
            foreach ($total_variedad as $valor) {
                $col++;
                setValueToCeldaExcel($objSheet, $columnas[$col] . $row, round($valor / 10000, 2));
                setBgToCeldaExcel($objSheet, $columnas[$col] . $row, 'e9ecef');   // gris
            }
