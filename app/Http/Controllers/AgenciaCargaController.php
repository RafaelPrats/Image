...
class AgenciaCargaController extends Controller
{
    ...
    public function listAgenciasCarga(Request $request){
        $listado = DB::table('agencia_carga as ac')->where('estado',$request->estado)
        ->where(function ($q) use ($request) {

            if ($request->busqueda != '')
                $q->Where(DB::raw("UPPER('ac.nombre')"), 'like', '%' . strtoupper(trim($request->busqueda)) . '%');

        })->orderBy('id_agencia_carga', 'desc')->get();
    ...
    public function storeAgenciaCarga(Request $request){
        ...
        if (!$valida->fails()) {
            ...
            if(empty($request->id_agencia_carga)){

                $objAgenciaCarga = new AgenciaCarga;
                $idAgeciaCarga = AgenciaCarga::orderBy('id_agencia_carga','desc')->first();

                $objAgenciaCarga->id_agencia_carga = isset($idAgeciaCarga) ? $idAgeciaCarga->id_agencia_carga+1 : 1;

            }else{

                $objAgenciaCarga = AgenciaCarga::find($request->id_agencia_carga);
            }
    ...
    public function storeClienteAgenciaCarga(Request $request)
    {
        $valida = Validator::make($request->all(), [
        ...
        if (!$valida->fails()) {
            ...
            try{
                ...
                $objClienteAgenciaCarga->id_cliente_agencia_carga = isset($cac->id_cliente_agencia_carga) ? $cac->id_cliente_agencia_carga + 1 : 1;
                $objClienteAgenciaCarga->id_cliente       = $request->id_cliente;
                $objClienteAgenciaCarga->id_agencia_carga = $request->id_agencia_carga;
                ...
                DB::commit();
                $msg = '<div class="alert alert-success text-center">Se ha guardado correctamente</div>';
                $success = true;
    ...
    public function storeContactoAgenciaCarga(Request $request)
    {
        $valida = Validator::make($request->all(), [
            'id_agencia_carga' => 'required',
            'nombre' => 'required',
        ...
        if (!$valida->fails()) {
            ...
            try{
                ...
                $clienteAgenciaCarga = ClienteAgenciaCarga::where('id_agencia_carga',$request->id_agencia_carga)->get();

                foreach($clienteAgenciaCarga as  $cac){
                    ...
                    $objContactoClienteAgenciaCarga->id_cliente_agencia_carga = $cac->id_cliente_agencia_carga;
                    $objContactoClienteAgenciaCarga->contacto = $request->nombre;
                    $objContactoClienteAgenciaCarga->correo = $request->correo;
                    $objContactoClienteAgenciaCarga->direccion = $request->direccion;
                    $objContactoClienteAgenciaCarga->save();
