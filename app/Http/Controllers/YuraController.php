...
class YuraController extends Controller
{
    public function inicio(Request $request)
    {
        if (count(getUsuario(Session::get('id_usuario'))->rol()->getSubmenusByTipo('C')) > 0) {
            return view('adminlte.inicio', [
                ...
                'costos_propagacion' => getIndicadorByName('C13')->valor,
                ...
                'costo_x_planta' => getIndicadorByName('C12')->valor,
    ...
    public function mostrar_indicadores_claves(Request $request)
    {
        if (count(getUsuario(Session::get('id_usuario'))->rol()->getSubmenusByTipo('C')) > 0) {
            ...
            if ($request->view == 'indicadores_rentabilidad_m2') {
                ...
                $sem_desde = getSemanaByDate(opDiasFecha('-', 364, $sem_hasta->fecha_inicial));   // 52 semana atras
            ...
            } elseif ($request->view == 'indicadores_claves_costos') {
                $sem_hasta = getSemanaByDate(opDiasFecha('-', 7, date('Y-m-d')));   // 1 semana atras
                $sem_desde = getSemanaByDate(opDiasFecha('-', 364, $sem_hasta->fecha_inicial));   // 52 semana atras
            ...
            } elseif ($request->view == 'indicadores_claves') {
                $sem_hasta = getSemanaByDate(opDiasFecha('-', 7, date('Y-m-d')));   // 1 semana atras
                $sem_desde = getSemanaByDate(opDiasFecha('-', 364, $sem_hasta->fecha_inicial));   // 52 semana atras
                ...
                $datos = [
                    ...
                    'productividad' => $variedad == '' ? getIndicadorByName('D8')->valor : getIndicadorByName('D8')->getVariedad($variedad->id_variedad)->valor,
            ...
            } elseif ($request->view == 'indicadores_costos_m2') {
                $datos = [
                    'costos_m2_mensual' => getIndicadorByName('C9')->valor,
                    'costos_m2_anual' => getIndicadorByName('C10')->valor,
                    'costos_m2_1_mes' => getIndicadorByName('C11')->valor,
                ];
            } elseif ($request->view == 'indicadores_ventas_m2') {
                $datos = [
                    'variedad' => $variedad,
            ...
            } elseif ($request->view == 'indicadores_datos_importantes') {
                $sem_hasta = getSemanaByDate(opDiasFecha('-', 7, date('Y-m-d')));   // 1 semana atras
                $sem_desde = getSemanaByDate(opDiasFecha('-', 364, $sem_hasta->fecha_inicial));   // 52 semana atras
            ...
            } elseif ($request->view == 'indicadores_costos_datos_importantes') {
                $sem_hasta = getSemanaByDate(opDiasFecha('-', 7, date('Y-m-d')));   // 1 semana atras
                $sem_desde = getSemanaByDate(opDiasFecha('-', 364, $sem_hasta->fecha_inicial));   // 52 semana atras
    ...
    public function select_filtro_variedad(Request $request)
    {
        ...
        return view('adminlte.partials.dashboard_x_variedad', [
            ...
            'costos_fijos' => getIndicadorByName('C7')->valor,
            'costos_regalias' => getIndicadorByName('C8')->valor,
            'costos_propagacion' => getIndicadorByName('C13')->getVariedad($variedad->id_variedad)->valor,
            'costos_m2_mensual' => getIndicadorByName('C9')->valor,
            'costos_m2_anual' => getIndicadorByName('C10')->valor,
            'costos_m2_1_mes' => getIndicadorByName('C11')->valor,
            'costo_x_planta' => getIndicadorByName('C12')->valor,
    ...
    public function ver_segundo_plano(Request $request)
    {
        return view('layouts.adminlte.partials.ver_segundo_plano', []);
    }

    public function listar_segundo_plano(Request $request)
    {
        $listado = PedidoProceso::where('fecha_registro', 'like', hoy() . '%')
            ->orderBy('fecha_registro')
            ->get();
        return view('layouts.adminlte.partials._listado_segundo_plano', [
            'listado' => $listado
        ]);
    }

    public function completar_pedido_proceso(Request $request)
    {
        try {
            $model = PedidoProceso::find($request->id);
            $model->estado = 'C';
            $model->progreso = 100;
            $model->cant_procesado = $model->total_procesar;
            $model->save();

            $success = true;
            $msg = 'Se han <b>COMPLETADO</b> el proceso correctamente';
        } catch (\Exception $e) {
            $success = false;
    ...
    public function delete_pedido_proceso(Request $request)
    {
        try {
            $model = PedidoProceso::find($request->id);
            $model->delete();
            ...
            $success = true;
            $msg = 'Se han <b>ELIMINADO</b> el proceso correctamente';
        } catch (\Exception $e) {
            $success = false;
            $msg = '<div class="alert alert-danger text-center">' .
                '<p> Ha ocurrido un problema al guardar la informacion al sistema</p>' .
