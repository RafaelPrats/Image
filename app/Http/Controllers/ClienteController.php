...
class ClienteController extends Controller
{
    ...
    {
        $busqueda = $request->has('busqueda') ? mb_strtoupper(espacios($request->busqueda)) : '';

        $listado = DB::table('cliente as cl')
            ->where('dc.estado', 1)
            ->join('detalle_cliente as dc', 'dc.id_cliente', '=', 'cl.id_cliente')
            ->leftJoin('pais as pa',  'dc.codigo_pais', '=', 'pa.codigo')
        ...
        if ($request->busqueda != '') $listado = $listado->Where(function ($q) use ($busqueda) {
            $q->Where(DB::raw("UPPER(dc.nombre)"), 'like', '%' . $busqueda . '%')
                ->orWhere(DB::raw("UPPER(dc.correo)"), 'like', '%' . $busqueda . '%')
                ->orWhere(DB::raw("UPPER(dc.ruc)"), 'like', '%' . $busqueda . '%')
                ->orWhere(DB::raw("UPPER(pa.nombre)"), 'like', '%' . $busqueda . '%');
        });
    ...
    public function add_clientes(Request $request)
    {
        ...
        isset($dataCliente->codigo_porcentaje_impuesto)
            ? $tipoImpuesto = TipoImpuesto::where('codigo', $dataCliente->codigo_porcentaje_impuesto)->get()
            : $tipoImpuesto = [];
    ...
    public function store_clientes(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            if (empty($request->id_cliente)) { //Guardar
                ...
                $objCliente->id_cliente = isset($cliente->id_cliente) ? $cliente->id_cliente + 1 : 1;
                $objCliente->estado  = 1;
                ...
                if ($objCliente->save()) {
                    ...
                    $detCliente = DetalleCliente::orderBy('id_detalle_cliente', 'desc')->first();
                    $objDetalleCliente->id_detalle_cliente = isset($detCliente->id_detalle_cliente) ? $detCliente->id_detalle_cliente + 1 : 1;
                    ...
                    $objDetalleCliente->codigo_porcentaje_impuesto    = 2;
                    ...
                    $objDetalleCliente->tipo_credito                  = $request->tipo_credito;
                    $objDetalleCliente->cod_pais_jire                 = $request->cod_pais_jire;
                    $objDetalleCliente->cod_ciudad_jire               = $request->cod_ciudad_jire;
                    $objDetalleCliente->impresion                     = $request->impresion;
                    $objDetalleCliente->nombre_empresa_etiqueta       = $request->impresion_nombre == 'true';
                    ...
                    if ($objDetalleCliente->save()) {
                        ...
                        foreach ($marcaciones as $m) {
                            $model = new ClienteDatoExportacion();
                            $model->id_dato_exportacion = $m->id_dato_exportacion;
                            $model->id_cliente = $objDetalleCliente->id_cliente;
                            $model->save();
                        }
    ...
    public function excel_clientes($objPHPExcel, $request)
    {
        ...
        if (count($listado) > 0) {
            ...
            for ($i = 0; $i < sizeof($listado); $i++) {
                ...
                $objSheet->getCell('D' . ($i + 4))->setValue(isset($pais) ? $pais->nombre : '');
    ...
    public function detalles_cliente(Request $request)
    {
        ...
        return view(
            'adminlte.gestion.postcocecha.clientes.partials.detalle',
            [
                'dataCliente'                => $dataCliente,
    ...
    public function ver_agencia_carga(Request $request)
    {
        ...
        return view(
            'adminlte.gestion.postcocecha.clientes.partials.select_agencias_carga',
            [
    ...
    public function store_agencia_carga(Request $request)
    {
        ...
        if (!$valida->fails()) {
            ...
            foreach ($request->data_agencias_carga as $x => $agencias_carga) {
                ...
                $existAgenciaCargoCliente = !empty($agencias_carga[1]) ?  ClienteAgenciaCarga::find($agencias_carga[1]) : '';
                ...
                if ($existAgenciaCargoCliente != '') {
                    ...
                    $cac = ClienteAgenciaCarga::orderBy('id_cliente_agencia_carga', 'desc')->first();
                    $objClienteAgenciaCarga->id_cliente_agencia_carga = isset($cac->id_cliente_agencia_carga) ? $cac->id_cliente_agencia_carga + 1 : 1;
                    $accion = "I";
                    $palabra = "Inserción";
                }
                ...
                if ($x == 0)
                    $idAcDefault = $objClienteAgenciaCarga->id_agencia_carga;
                ...
                if ($objClienteAgenciaCarga->save()) {
                    ...
                    foreach ($request->contactos[$x] as $contacto) {
                        ...
                        $ccac = ContactoClienteAgenciaCarga::orderBy('id_contactos_cliente_agenciacarga', 'desc')->first();
                        $objContactoClienteAgenciaCarga->id_contactos_cliente_agenciacarga = isset($ccac->id_contactos_cliente_agenciacarga) ? $ccac->id_contactos_cliente_agenciacarga + 1 : 1;
            ...
            if ($idAcDefault != null) {
                ...
                foreach ($pedidos as $ped) {
                    $objDetPed = DetallePedido::where('id_pedido', $ped->id_pedido)->get();
                    foreach ($objDetPed as $depPed) {
                        $obj = DetallePedido::find($depPed->id_detalle_pedido);
                        $obj->id_agencia_carga = $idAcDefault;
                        $obj->save();
                    }
                }
            }
    ...
    public function ver_contactos_clientes(Request $request)
    {
        ...
        return view(
            'adminlte.gestion.postcocecha.clientes.partials.inputs_contactos_clientes',
            [
                'cantTr' => $request->cant_tr,
            ]
        );
        ...
        if (!$valida->fails()) {
            ...
            foreach ($request->data_contactos as $contactos) {
                ...
                if ($existContacto != '') {
                    ...
                    $contacto = Contacto::orderBy('id_contacto', 'desc')->first();
                    $objClienteContacto->id_contacto = isset($contacto->id_contacto) ? $contacto->id_contacto + 1 : 1;
                    $accion = "I";
                    $palabra = "Inserción";
                }
                ...
                if ($objClienteContacto->save()) {
                    ...
                    if ($accion === 'I') {
                        ...
                        $detContacto = DetalleClienteContacto::orderBy('id_detalle_cliente_contacto', 'desc')->first();
                        $objDetalleClienteContacto->id_detalle_cliente_contacto = isset($detContacto->id_detalle_cliente_contacto) ? $detContacto->id_detalle_cliente_contacto + 1 : 1;
    ...
    public function agregar_consignatario(Request $request)
    {
        return view('adminlte.gestion.postcocecha.clientes.forms.add_consignatario', [
            'id_cliente' => $request->id_cliente,
            'dataPais' => Pais::orderBy('nombre', 'asc')->get(),
            'clienteConsignatario' => ClienteConsignatario::where([
                //['estado',true],
                ['id_cliente', $request->id_cliente]
            ])->get()
        ]);
    ...
    public function store_cliente_consignatario(Request $request)
    {
        ...
        try {
            ...
            if ($request->arr_consignatarios != '')
                foreach (json_decode($request->arr_consignatarios) as $consignatario) {

                    if ($request->importar_data) {
                        ...
                        $objClienteConsignatario->default = true;
                        $objClienteConsignatario->save();
                    } else {

                        if ($consignatario->id_consignatario == '') {
                            $objConsignatario = new Consignatario;
                        } else {
                            $objConsignatario = Consignatario::find($consignatario->id_consignatario);
                        }

                        $objConsignatario->nombre = $consignatario->nombre;
                        $objConsignatario->direccion = isset($consignatario->direccion) ? $consignatario->direccion : '';
                        $objConsignatario->telefono = $consignatario->telefono;
                        $objConsignatario->correo = $consignatario->correo;
                        $objConsignatario->codigo_pais = $consignatario->pais;
                        $objConsignatario->ciudad = $consignatario->ciudad;
                        $objConsignatario->identificacion = $consignatario->identificacion;
                        $objConsignatario->save();
                        ...
                        if ($consignatario->id_consignatario == '') {
                            ...
                            $objClienteConsignatario = new ClienteConsignatario;
                            $objClienteConsignatario->id_cliente = $request->id_cliente;
                            $objClienteConsignatario->id_consignatario = $model->id_consignatario;
                            $objClienteConsignatario->default = true;
                            $objClienteConsignatario->save();
                        }
                    }
            ...
            $success = true;
            $msg =  '<div class="alert alert-success text-center">' .
                '<p> Se ha guardado el consignatario exitosamente</p>'
                . '</div>';
            ...
            $success = false;
            $msg = '<div class="alert alert-warning text-center">' .
                '<p> Ha ocurrido un problema al guardar la información al sistema ' . $e->getMessage() . ' En la línea ' . $e->getLine() . ' del archivo ' . $e->getFile() . '</p>'
                . '</div>';
        }
    ...
    public function form_importar_cliente(Request $request)
    {
        return view('adminlte.gestion.postcocecha.clientes.forms.form_importar_clientes');
    }

    public function importar_cliente(Request $request)
    {
        $valida = Validator::make($request->all(), [
        ...
        if (!$valida->fails()) {
            ...
            try {
                ...
                $hojaClientes = $document->getSheet(0)->toArray(null, true, true, true);
                $hojaContactos = $document->getSheet(1)->toArray(null, true, true, true);

                for ($i = 2; $i <= count($hojaClientes); $i++) {

                    $validArrayCliente = $this->validaClientes($hojaClientes[$i]);

                    if (!is_string($validArrayCliente)) {
                        ...
                        $objCliente->estado  = 1;
                        $objCliente->fc = false;
                        $objCliente->csv = false;
                        $objCliente->le = false;
                        $objCliente->dc = false;
                        $objCliente->fc_sri = false;
                        $objCliente->save();

                        $pais = Pais::where('nombre', trim($hojaClientes[$i]['G']))->first();
                        if (!isset($pais))
                            throw new Exception('El país ' . $hojaClientes[$i]['G'] . ' no existe en el sistema');

                        $impuesto = Impuesto::where('nombre', trim(strtoupper($hojaClientes[$i]['E'])))->first();
                        if (!isset($impuesto))
                            throw new Exception('El impuesto ' . $hojaClientes[$i]['E'] . ' no existe en el sistema');

                        $porcentajeImpuesto = TipoImpuesto::where('porcentaje', trim(strtoupper($hojaClientes[$i]['F'])))->first();
                        if (!isset($porcentajeImpuesto))
                            throw new Exception('El porcentaje de impuesto ' . $hojaClientes[$i]['F'] . ' no existe en el sistema');

                        $tipoIdentificacion = TipoIdentificacion::where('nombre', trim(strtoupper($hojaClientes[$i]['B'])))->first();
                        if (!isset($tipoIdentificacion))
                            throw new Exception('El tipo de identificación ' . $hojaClientes[$i]['B'] . ' no existe en el sistema');

                        $marca = Marca::where('nombre', trim(strtoupper($hojaClientes[$i]['M'])))->first();
                        if (!isset($marca))
                            throw new Exception('La marca ' . $hojaClientes[$i]['M'] . ' no existe en el sistema');

                        $objDetalleCliente = new DetalleCliente;
                        $detCliente = DetalleCliente::orderBy('id_detalle_cliente', 'desc')->first();
                        $objDetalleCliente->id_detalle_cliente = isset($detCliente->id_detalle_cliente) ? $detCliente->id_detalle_cliente + 1 : 1;
                        $objDetalleCliente->nombre = $hojaClientes[$i]['A'];
                        $objDetalleCliente->ruc = $hojaClientes[$i]['C'];
                        $objDetalleCliente->codigo_pais = $pais->codigo;
                        $objDetalleCliente->provincia = $hojaClientes[$i]['H'];
                        $objDetalleCliente->correo = $hojaClientes[$i]['I'];
                        $objDetalleCliente->telefono = $hojaClientes[$i]['D'];
                        $objDetalleCliente->direccion = $hojaClientes[$i]['L'];
                        $objDetalleCliente->id_cliente = $objCliente->id_cliente;
                        $objDetalleCliente->codigo_porcentaje_impuesto = $porcentajeImpuesto->codigo;
                        $objDetalleCliente->codigo_identificacion = $tipoIdentificacion->codigo;
                        $objDetalleCliente->codigo_impuesto = $impuesto->codigo;
                        $objDetalleCliente->puerto_entrada = $hojaClientes[$i]['J'];
                        $objDetalleCliente->tipo_credito = $hojaClientes[$i]['K'];
                        $objDetalleCliente->id_marca = $marca->id_marca;
                        $objDetalleCliente->save();
                    } else {

                        $msg .= '<p>' . $validArrayCliente . ' en la línea ' . $i . ' corrjalas y vuelva a cargar sólo esas líneas</p>';
                    }
                }

                for ($j = 2; $j <= count($hojaContactos); $j++) {

                    $validArrayContacto = $this->validaContactos($hojaContactos[$j]);

                    if (!is_string($validArrayContacto)) {

                        $detCliente = DetalleCliente::where(function ($q) use ($hojaContactos, $j) {

                            $q->where('nombre', strtoupper(trim($hojaContactos[$j]['A'])))
                                ->orWhere('nombre', strtolower(trim($hojaContactos[$j]['A'])));
                        })->where('estado', true)->first();

                        if (!isset($detCliente))
                            throw new Exception('El cliente ' . $hojaContactos[$j]['A'] . ' no existe en el sistema');

                        $objClienteContacto = new Contacto;
                        $contacto = Contacto::orderBy('id_contacto', 'desc')->first();
                        $objClienteContacto->id_contacto = isset($contacto->id_contacto) ? $contacto->id_contacto + 1 : 1;
                        $objClienteContacto->nombre = $hojaContactos[$j]['B'];
                        $objClienteContacto->correo = $hojaContactos[$j]['C'];
                        $objClienteContacto->telefono = $hojaContactos[$j]['D'];
                        $objClienteContacto->direccion = $hojaContactos[$j]['E'];
                        $objClienteContacto->save();

                        $objDetalleClienteContacto = new DetalleClienteContacto;
                        $detContacto = DetalleClienteContacto::orderBy('id_detalle_cliente_contacto', 'desc')->first();
                        $objDetalleClienteContacto->id_detalle_cliente_contacto = isset($detContacto->id_detalle_cliente_contacto) ? $detContacto->id_detalle_cliente_contacto + 1 : 1;
                        $objDetalleClienteContacto->id_detalle_cliente = $detCliente->id_detalle_cliente;
                        $objDetalleClienteContacto->id_contacto = $objClienteContacto->id_contacto;
                        $objDetalleClienteContacto->save();
                    } else {
                    }
                }
                ...
                $msg = '<div class="alert alert-success text-center">' .
                    '<p> Se han guardado los nuevos clientes con éxito </p>'
                    . '</div>';
            } catch (\Exception $e) {
    ...
    public function validaClientes($data)
    {
        if ($data['A'] === null) return 'El campo NOMBRE es obligatorio';
        if ($data['B'] === null) return 'El campo TIPO IDENTIFICACION es obligatorio';
        if ($data['C'] === null) return 'El campo NUMERO IDENTIFICACION es obligatorio';
        if ($data['D'] === null) return 'El campo TELEFONO es obligatorio';
        if ($data['E'] === null) return 'El campo TIPO IMPUESTO es obligatorio';
        if ($data['F'] === null) return 'El campo PORCENTAJE IMPUESTO es obligatorio';
        if ($data['G'] === null) return 'El campo PAIS es obligatorio';
        if ($data['H'] === null) return 'El campo CIUDAD es obligatorio';
        if ($data['I'] === null) return 'El campo CORREO es obligatorio';
        if ($data['J'] === null) return 'El campo PUERTO ENTRADA es obligatorio';
        if ($data['K'] === null) return 'El campo TIEMPO CREDITO es obligatorio';
        if ($data['L'] === null) return 'El campo DIRECCION es obligatorio';

        return true;
    }

    public function validaContactos($data)
    {
        if ($data['A'] === null) return 'El campo CLIENTE es obligatorio';
        if ($data['B'] === null) return 'El campo NOMBRE es obligatorio';
        if ($data['C'] === null) return 'El campo CORREO IDENTIFICACION es obligatorio';
        if ($data['D'] === null) return 'El campo TELEFONO es obligatorio';
        if ($data['E'] === null) return 'El campo DIRECCION es obligatorio';

        return true;
    }

    public function formato_carga_clientes()
    {
        //---------------------- EXCEL --------------------------------------
        $objPHPExcel = new PHPExcel;
        $objPHPExcel->getDefaultStyle()->getFont()->setName('Calibri');
        ...
        $this->excel_formato_carga_masiva_clientes($objPHPExcel);
        ...
        header("Content-Type: application/octet-stream");
        header("Content-Type: application/download");
        header('Content-Disposition:inline;filename="Carga masiva clienter.xlsx"');
        header("Content-Transfer-Encoding: binary");
        header("Last-Modified: " . gmdate("D, d M Y H:i:s") . " GMT");
        header("Cache-Control: must-revalidate, post-check=0, pre-check=0");
        header("Pragma: no-cache");
    ...
    public function excel_formato_carga_masiva_clientes($objPHPExcel)
    {
        $objSheet = new PHPExcel_Worksheet($objPHPExcel, 'Clientes');
        $objPHPExcel->addSheet($objSheet, 0);
        ...
        $objSheet->getCell('B1')->setValue('TIPO IDENTIFICACIÓN');
        $objSheet->getCell('C1')->setValue('NUMERO IDENTIFICACIÓN');
        $objSheet->getCell('D1')->setValue('TELEFONO');
        $objSheet->getCell('E1')->setValue('TIPO IMPUESTO');
        $objSheet->getCell('F1')->setValue('PORCENTAJE IMPUESTO');
        $objSheet->getCell('G1')->setValue('PAÍS');
        $objSheet->getCell('H1')->setValue('CIUDAD');
        $objSheet->getCell('I1')->setValue('CORREO');
        $objSheet->getCell('J1')->setValue('PUERTO DE ENTRADA');
        $objSheet->getCell('K1')->setValue('TIEMPO DE CRÉDITO (DÍAS)');
        $objSheet->getCell('L1')->setValue('DIRECCIÓN');
        $objSheet->getCell('M1')->setValue('MARCA');
        ...
        $objSheet2 = new PHPExcel_Worksheet($objPHPExcel, 'Contactos');
        $objPHPExcel->addSheet($objSheet2, 1);

        $objSheet2->getStyle('A1:E1')->getFont()->setBold(true)->setSize(12);
        $objSheet2->getStyle('A1:E1')->getAlignment()->setHorizontal(PHPExcel_Style_Alignment::HORIZONTAL_CENTER);

        $objSheet2->getCell('A1')->setValue('CLIENTE (ESCRIBIR EL NOMBRE EXACTO DE LA PRIMERA HOJA)');
        $objSheet2->getCell('B1')->setValue('NOMBRE');
        $objSheet2->getCell('C1')->setValue('CORREO');
        $objSheet2->getCell('D1')->setValue('TELEFONO');
        $objSheet2->getCell('E1')->setValue('DIRECCIÓN');

        $objSheet2->getColumnDimension('A')->setAutoSize(true);
        $objSheet2->getColumnDimension('B')->setAutoSize(true);
        $objSheet2->getColumnDimension('C')->setAutoSize(true);
        $objSheet2->getColumnDimension('D')->setAutoSize(true);
        $objSheet2->getColumnDimension('E')->setAutoSize(true);
    }

    public function eliminar_cliente_consignatario(Request $request)
    {
        ClienteConsignatario::destroy($request->id_cliente_consignatario);

        return [
            'success' => true,
            'mensaje' => '<div class="alert alert-success text-center">' .
                '<p> Se eliminado el consignatario con éxito </p>'
                . '</div>'
        ];
    }
}
