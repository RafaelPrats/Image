...
class dbController extends Controller
{
    ...
    public function store_indicador(Request $request)
    {
        ...
        if (!$valida->fails()) {
            $model = new Indicador();
            $indicador = Indicador::orderBy('id_indicador','desc')->first();
            $model->id_indicador = isset($indicador) ? $indicador->id_indicador + 1 : 1;
        ...
        if (!$valida->fails()) {
            ...
            foreach ($request->datos as $dato) {
                try {
                    ...
                    $intervaloIndicador = IntervaloIndicador::orderBy('id_intervalo_indicador','desc')->first();
                    $objIntervaloIndicador->id_intervalo_indicador = isset($intervaloIndicador->id_intervalo_indicador) ? $intervaloIndicador->id_intervalo_indicador + 1 : 1;
