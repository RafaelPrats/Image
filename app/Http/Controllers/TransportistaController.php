   ...
   public function store_transportista(Request $request){
       ...
       if (!$valida->fails()) {
           ...
           if(empty($request->id_transportista)){
               ...
               $t = Transportista::orderBy('id_transportista','desc')->first();
               $objTransportista->id_transportista = isset($t->id_transportista) ? $t->id_transportista + 1 : 1;
               $palabra = 'Inserción';
               $accion   = 'I';
           }else{
   ...
   public function store_camion(Request $request){
       ...
       if (!$valida->fails()) {
           ...
           if(empty($request->id_camion)){
                $objCamion = new Camion;
                $camion = Camion::orderBy('id_camion','desc')->first();
                $objCamion->id_camion = isset($camion->id_camion) ? $camion->id_camion + 1 : 1;
                $palabra = 'Inserción';
                $accion   = 'I';
           }else{
           ...
           $objCamion->modelo = $request->modelo;
           $objCamion->placa = $request->placa;
           $objCamion->id_transportista = $request->id_transportista;
