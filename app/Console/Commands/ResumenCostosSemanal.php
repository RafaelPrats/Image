...
class ResumenCostosSemanal extends Command
{
    ...
    public function handle()
    {
        ...
        foreach ($semanas as $pos_s => $semana) {
            dump('sum: ' . ($pos_s + 1) . '/' . count($semanas));
            ...
            if ($resumen == '') {   // es nuevo
                ...
                $resumenCostosSemanal = ModelResumen::orderBy('id_resumen_costos_semanal','desc')->first();
                $resumen->id_resumen_costos_semanal = isset($resumenCostosSemanal->id_resumen_costos_semanal) ? $resumenCostosSemanal->id_resumen_costos_semanal + 1 : 1;
