                    ...
                    foreach ($array_semanas as $pos_sem => $codigo) {
                        if ($restriccion_parametro == 0 || ($restriccion_parametro == 1 && $pos_sem <= 15)) {
                            ...
                            if ($proy == ''){
                                ...
                                $proyModuloSemana= ProyeccionModuloSemana::orderBy('id_proyeccion_modulo_semana','desc')->first();
                                $proy->id_proyeccion_modulo_semana= isset($proyModuloSemana->id_proyeccion_modulo_semana) ? $proyModuloSemana->id_proyeccion_modulo_semana+1 : 1;
                            }
                            ...
                            if (in_array($data['tipo'], ['S', 'P', 'T', 'Y'])) {
                                if ($data['tabla'] == 'C') {
                                    ...
                                    $last_ciclo = $mod->getCicloByFecha(date('Y-m-d'));
                                    ...
                                    $proy->activo = 0;
                                    $proy->area = $last_ciclo != '' ? $last_ciclo->area : $mod->area;
