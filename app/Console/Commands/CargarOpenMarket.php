...
class CargarOpenMarket extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cargar:open-market';
     ...
     * @var string
     */
    protected $description = 'Carga pedido Open Marker';
        ...
        try{
            ...
            for($i=2; $i <= count($hojaPedido); $i++){
                ...
                if(trim($hojaPedido[$i]['A']) != 'STANDING'){
                                    //CLIENTE                   SEMANA                       DIA
                    $clientes[trim($hojaPedido[$i]['G'])][trim($hojaPedido[$i]['A'])][trim($hojaPedido[$i]['B'])][] = [
            ...
            foreach($clientes as $idcliente => $cli){
                ...
                foreach($cli as $sem => $semana){

                    foreach($semana as $d => $dia){

                        $fechas=[];

                        $week=substr(trim($sem),2,4);
                        $year='20'.substr(trim($sem),0,2);
                        $date= Carbon::now();
                        $date->setISODate($year,$week);
                        $comienzo = Carbon::parse($date->startOfWeek()->toDateString());
                        $fin = Carbon::parse($date->endOfWeek()->toDateString());

                        for($i = $comienzo; $i <= $fin; $i->addDay())
                            if($i->dayOfWeek == $d)
                                $fechas[]=$i->toDateString();
                                    ...
                                    if(isset($planta)){
                                        ...
                                        if(isset($variedad)){
                                            ...
                                            if(isset($cpe)){
                                                ...
                                                $alertas.= 'El cliente '.$cliente->nombre.' con el código '.$idcliente.' No tiene la siguiente especificacion VARIEDAD '.$planta->nombre.' TIPO '.$variedad->nombre.' CAJA '.trim($det['P']).' RAMOS X CAJA '.trim($det['R']).' TALLOS X RAMO '.trim($det['S']).' LONGITUD '.trim($det['N']).' para el pedido de los '.$days[trim($d)].' en la fila '.trim($det['V'])."\n\n";
                                                $z = false;
                                                //break 2;
