        ...
        if ($desde_par <= $hasta_par) {
            ...
            foreach (Variedad::All() as $variedad) {
                foreach ($array_semanas as $sem) {
                    ...
                    $model = Regalias::find(isset($model) ? $model->id_regalias : 0);
                    if ($model == '') {
                        $model = new Regalias();
                        $regalia = Regalias::orderBy('id_regalias','desc')->first();
                        $model->id_regalias = isset($regalia) ? $regalia->id_regalias + 1 : 1;
