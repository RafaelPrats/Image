...
class jobCopiarPedido implements ShouldQueue
{
     ...
     * @return void
     */
    public $timeout = 3600; // 1 hora en segundos

    protected $pedido_original;
    protected $fecha;
    protected $total_progreso;
    protected $usuario;
    protected $ip;

    public function __construct($pedido_original, $fecha, $total_progreso, $usuario, $ip)
    {
        $this->pedido_original = $pedido_original;
        $this->fecha = $fecha;
    ...
    {
        $pedOriginal = Pedido::find($this->pedido_original);
        /* REGISTRO en TABLA PEDIDO_PROCESO */
        $existe_proceso = DB::table('pedido_proceso')
            ->where('id_pedido', $this->pedido_original)
            ->where('estado', 'P')  // Pendiente
            ->where('tipo_proceso', 'C')    // Copiar Pedido
            ->get()
            ->first();
        if ($existe_proceso == '') {
            $proceso = new PedidoProceso();
            $proceso->id_pedido = $this->pedido_original;
            $proceso->estado = 'P';
            $proceso->descripcion = 'COPIAR PEDIDO id' . $pedOriginal->id_pedido . ', del CLIENTE: ' . $pedOriginal->cliente->detalle()->nombre . ', fecha: ' . $pedOriginal->fecha_pedido . ' a ' . $this->total_progreso . ' fechas';
            $proceso->id_usuario = $this->usuario;
            $proceso->tipo_proceso = 'C';
        ...
        $bitacora->accion = 'I';
        $bitacora->id_usuario = $this->usuario;
        $bitacora->observacion = 'COPIAR PEDIDO del cliente ' . $ped->cliente->detalle()->nombre . ', fecha ' . $ped->fecha_pedido . ' usando como GUIA: id=' . $pedOriginal->id_pedido . ', con fecha: ' . $pedOriginal->fecha_pedido;
        $bitacora->ip = $this->ip;
