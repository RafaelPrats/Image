...
<head>
    ...
    <link rel="stylesheet" href="{{ url('css/yura_estilos.css?v=1.0.1') }}">

    @yield('css_inicio')
    @yield('script_inicio')
    <!-- Hotjar Tracking Code for Nintanga -->
    <script>
...
<body class="hold-transition skin-black sidebar-mini">
    ...
    <script src="{{ asset('js/yura_script.js?v=1.0.1') }}"></script>
    ...
    <script>
        ...
        function cargar_manual_usuario() {
            datos = {
                vista_actual: vista_actual,
            };
            get_jquery('{{ url('cargar_manual_usuario') }}', datos, function(retorno) {
                $('#div_manual_usuario').html(retorno);
            });
        }
        ...
        function post_jquery(url, datos, success, div = false, retorna = false) {
        ...
        function cargar_url(url, accion) {

            if (typeof $(accion).parent().parent().parent().attr('class') != 'undefined') {
                let classNoHide = $(accion).parent().parent().parent().attr('class').split(' ')[1]
                localStorage.setItem('classNoHide', classNoHide)
            }
        ...
        function buscar_notificaciones(tipo, push = true) {
            ...
            $.post('notificaciones/buscar_notificaciones', datos, function(retorno) {
                ...
                if (retorno.cant_news > 0) {
                    ...
                    if (parseInt($('#input_cant_not').val()) < retorno.cant_news)
                        beep_notificar();

                    $('#link_not').html('<i class="fa fa-fw fa-star"></i>');
                    $('#input_cant_not').val(retorno.cant_news);
        ...
        $(window).on('load', function() {
            ...
            $("li." + location.pathname.slice(1)).parent().css('display', 'block');
            $("li." + location.pathname.slice(1)).parent().parent().addClass('menu-open')
            $("li." + localStorage.getItem('classNoHide')).removeClass('hide');
        ...
        function get_distribuciones_pendientes() {
            datos = {};
            get_jquery('{{ url('distribucion_cosecha/get_distribuciones_pendientes') }}', datos, function(retorno) {
                $('#ul_distribuciones_pendientes').html(retorno);
            }, 'ul_distribuciones_pendientes');
        }

        function buscar_cambios_diarios() {
            datos = {};
            get_jquery('{{ url('proyectos/buscar_cambios_diarios') }}', datos, function(retorno) {
                $('#list_cambios').html(retorno);
            }, 'list_cambios');
        }
        ...
        function cargar_submenu_crm() {
            datos = {};
            get_jquery('{{ url('cargar_submenu_crm') }}', datos, function(retorno) {
                $('#div_submenu_crm').html(retorno);
            }, 'ul_navbar_superior');
        }
        ...
        function calcularCamas(m2) {
            return Math.round((m2 / 48) * 100) / 100;
        }
        ...
        function getDifHoras(hora_inicio, hora_final) {
        ...
        function ver_segundo_plano() {
            datos = {};
            get_jquery('{{ url('ver_segundo_plano') }}', datos, function(retorno) {
                modal_view('modal_view_ver_segundo_plano', retorno,
                    '<i class="fa fa-fw fa-cogs"></i> Pedidos en Proceso', true, false,
                    '{{ isPC() ? '95%' : '' }}');
            });
        }
    </script>
