<form id="form_mover_fecha">
    <div class="input-group">
        ...
        <input type="text" readonly id="saldo" name="saldo" value="{{number_format($apertura->cantidad_disponible)}}" class="form-control input-yura_default">
        <div class="input-group-addon bg-yura_dark">
            Mover
        </div>
        <input type="number" onkeypress="return isNumber(event)" id="mover" name="mover" value="0" class="form-control input-yura_default"
               max="{{$apertura->cantidad_disponible}}" min="1" onchange="calcular_saldo()">
        <div class="input-group-addon bg-yura_dark">
            Para
        </div>
        <input type="date" required id="fecha_mover" name="fecha_mover" value="{{date('Y-m-d')}}" class="form-control input-yura_default">
        <div class="input-group-btn">
            <button type="button" class="btn btn-yura_dark" onclick="store_mover_fecha()">
