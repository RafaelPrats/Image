<script>
    ...
    function monitoreo_calibres(fecha = '') {
        datos = {
            fecha: fecha != '' ? fecha : $('#fecha_verde_search').val()
        };
        get_jquery('{{url('clasificacion_verde/monitoreo_calibres')}}', datos, function (retorno) {
            modal_view('modal-view_monitoreo_calibres', retorno, '<i class="fa fa-fw fa-tree"></i> Monitoreo de Calibres', true,
                false, '95%');
        });
    }
    ...
    /* -------------------------------------------------------------- */
    function add_new_verde() {
        datos = {
            fecha: $('#fecha_verde_search').val(),
        };
        get_jquery('{{url('clasificacion_verde/add_new_verde')}}', datos, function (retorno) {
            modal_view('modal-view_add_new_verde', retorno, '<i class="fa fa-fw fa-plus"></i> Añadir clasificación en verde', true, false, '{{isPC() ? '75%' : ''}}');
        })
    }
</script>