<div id="table_clasificaciones">
    @if($verde != '')
        <div style="overflow-x: scroll; width: 100%">
            <table width="100%" class="table-responsive table-bordered sombra_estandar"
                   style="border-color: #9d9d9d; border-radius: 18px 18px 0 0"
                ...
                <tr onmouseover="$(this).css('background-color','#add8e6')" onmouseleave="$(this).css('background-color','')"
                    ...
                    {{--<td style="border-color: #9d9d9d" class="text-center">
                        {{$getTotalRamosEstandar}}
                    </td>--}}
                    {{--<td style="border-color: #9d9d9d" class="text-center">
                        {{round($getTotalRamosEstandar / $ramos_x_caja, 2)}}
                    </td>--}}
                    <td style="border-color: #9d9d9d" class="text-center">
                        @if($getTotalRamosEstandar > 0)
                            {{round($total_tallos /
                            $getTotalRamosEstandar,2)}}
                        @else
                            0
                        @endif
                    </td>
                                ...
                                @foreach($verde->variedades() as $variedad)
                                    <tr>
                                                ...
                                                <tr>
                                                    ...
                                                    @foreach($unitarias as $unitaria)
                                                        <th style="border-color: #9d9d9d; background-color: {{explode('|',$unitaria->color)[0]}}; color: {{explode('|',$unitaria->color)[1]}}"
                                                            class="text-center" width="{{100 / count($unitarias)}}%">
                                        ...
                                        {{--<td style="border-color: #9d9d9d; padding: 10px; margin-bottom: 0" class="text-center" width="10%">
                                            {{$getTotalRamosEstandarByVariedad}}
                                        </td>
                                        <td style="border-color: #9d9d9d; padding: 10px; margin-bottom: 0" class="text-center" width="10%">
                                            {{round($getTotalRamosEstandarByVariedad /
                                            $ramos_x_caja, 2)}}
                                        </td>--}}
