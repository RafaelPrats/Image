<table style="width:1070px; position: relative; left: -20px;" cellspacing="0">
    <thead>
        <tr>
            <td colspan="2">{{ $data['despacho']->fecha_despacho }}</td>
            <td colspan="6">CONDUCTOR: Sr. {{ $data['despacho']->conductor->nombre }}</td>
        </tr>
        <tr>
            <td><b>N°</b></td>
            <td><b>CARGERAS</b></td>
    ...
    <tbody>
        @php
            ...
            $total_piezas = 0;
        @endphp
        @foreach ($data['despacho']->detalles as $x => $det_desp)
            @php
                $pedido = $det_desp->pedido;
                $total_piezas += $det_desp->cantidad;

                $detalle_pedido = yura\Modelos\DetallePedido::where('id_pedido', $pedido->id_pedido)->first();

                $marcaciones = yura\Modelos\DetallePedidoDatoExportacion::where('id_detalle_pedido', $detalle_pedido->id_detalle_pedido)
                ->get()
                ->pluck('valor')
                ->toArray();
            ...
            <tr>
                ...
                <td>{{ $pedido->cliente->detalle()->nombre }}</td>
                <td>{{ isset($pedido->envios[0]) && isset($pedido->envios[0]->consignatario) ? $pedido->envios[0]->consignatario->nombre : '' }}
                </td>
                <td>
                    @php
                    if (isset($marcaciones) && !empty($marcaciones)){
                        print implode(' - ', $marcaciones); 
                    }
                    @endphp
                </td>
                <td>{{ $pedido->packing }}</td>
                <td>{{ $det_desp->cantidad }}</td>
                <td>
                    @php
                        $caja_full = 0;
                        ...
                        $total_fulles += $caja_full;
                    @endphp
                    {{ $caja_full }}
                </td>
                <td>{{ isset($pedido->envios[0]) ? $pedido->envios[0]->guia_madre : '' }}</td>
                <td>{{ isset($pedido->envios[0]) ? $pedido->envios[0]->guia_hija : '' }}</td>
            </tr>
        @endforeach
        <tr>
            <td colspan="6" style="text-align: right">Total:</td>
            <td style="text-align: left">{{ $total_piezas }}</td>
            <td style="text-align: left">{{ $total_fulles }}</td>
            <td colspan="2"></td>
        </tr>
    </tbody>
</table>
