<form id="form_despacho_{{ $cant_form + 1 }}" style="overflow-x: auto" class="form-horizontal sombra_estandar">
    <table width="100%" class="table-responsive table-bordered" style=" border-color: white;margin-top:20px"
        id="table_despacho_{{ $cant_form + 1 }}">
        <tr>
            ...
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <select id="id_transportista" name="id_transportista" style="width: 100%;border: none;" required
                    onchange="busqueda_camiones_conductores('form_despacho_{{ $cant_form + 1 }}')">
            ...
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <select id="id_camion" style="width: 100%;border: none"
                    onchange="busqueda_placa_camion('form_despacho_{{ $cant_form + 1 }}')" required></select>
            </td>
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <b>Placa</b>
            </td>
        ...
        <tr>
            ...
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle" colspan="3">
                <input type="text" id="responsable" name="responsable" onkeyup="duplicar_nombre(this)"
                    style="width: 100%;border: none;"
                    value="{{ isset($resp_transporte->resp_transporte) ? $resp_transporte->resp_transporte : '' }}"
                    required>
            </td>
        </tr>
        <tr>
            ...
            <td style="border-color: #9d9d9d;vertical-align: middle">
                <input type="text" id="semana" name="semana"
                    value="{{ getSemanaByDate(now()->toDateString())->codigo }}" readonly
                    style="width: 100%;border: none;" required>
            </td>
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <b>Rango Tmp</b>
        ...
        <tr>
            ...
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <input value="{{ $cant_form + 1 }}" type="number" id="n_viaje" name="n_viaje"
                    style="width: 100%;border: none;" required>
            </td>
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                <b>Hora de salida</b>
        ...
        <tr>
            <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle" colspan="8"><b>SELLOS</b>
            </td>
        </tr>
        ...
        @php
            ...
            foreach ($pedidos as $p) {
                if (
                    !in_array(
                        $p->detalles[0]->agencia_carga->id_agencia_carga,
                        array_column($sellos, 'id_agencia_carga'),
                    )
                ) {
                    $sellos[] = [
                        'id_agencia_carga' => $p->detalles[0]->agencia_carga->id_agencia_carga,
                        'nombre_agencia' => $p->detalles[0]->agencia_carga->nombre,
                    ];
                }
            }
        ...
        @foreach ($sellos as $x => $s)
            <tr class="tr_sellos" data-id_agencia_carga="{{ $s['id_agencia_carga'] }}">
                <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                    {{ $s['nombre_agencia'] }}
                </td>
                @for ($i = 0; $i < 7; $i++)
                    <td class="text-center" style="border-color: #9d9d9d;vertical-align: middle">
                        <input type="text" class="sello" name="sello" style="width: 100%;border: none;"
                            {{-- {{$i==0 ? "required" : ""}} --}}>
                    </td>
                @endfor
            </tr>
...
<script>
    ...
    add_pedido_piezas('table_despacho_1');
    ...
    function add_pedido_piezas(tabla) {
        ...
        datos = {
            cant_form: 1, //tabla.split("_")[2],
            //secuencial : n_pedido,//cant_tr_pedido_piezas+1,
        ...
        $.get('{{ url('despachos/add_pedido_piezas') }}', datos, function(retorno) {
            ...
            if (datos.cant_form == 1 && tr_pedido_piezas < 2) {
                ...
                $("#sb_1_1").val(parseInt($("#sb_box").html()));
                $("#piezas_x_camion_1").html(parseInt($("#full_box").html()) + parseInt($("#half_box").html()) +
                    parseInt($("#cuarto_box").html()) + parseInt($("#sexto_box").html()) + parseInt($(
                        "#octavo_box").html()) + parseInt($("#sb_box").html()));
            } else {
            ...
            for (i = 1; i <= datos.cant_form; i++) {
                for (x = 1; x <= $("input.caja").length; x++) {
                    ...
                    if (parseInt($("#cuarto_box").html()) === 0) $("#cuarto_" + i + "_" + x).attr('readonly',
                        true);
                    if (parseInt($("#sexto_box").html()) === 0) $("#sexto_" + i + "_" + x).attr('readonly',
                        true);
                    if (parseInt($("#octavo_box").html()) === 0) $("#octavo_" + i + "_" + x).attr('readonly',
                        true);
                    if (parseInt($("#sb_box").html()) === 0) $("#sb_" + i + "_" + x).attr('readonly',
                        true);
                }
            }

            $("#" + tabla + " td#td_piezas").attr('rowspan', arr_pedidos.length)
            $("#" + tabla + " td#td_btn").attr('rowspan', arr_pedidos.length)

            $.each($("select.pedido"), function(i, j) {

                $(j).find("option[value=" + arr_pedidos[i] + "]").attr('selected', true)
                obetener_piezas(1, (i + 1), j)
            })
    ...
    function obetener_piezas(form, sec, select) {
        ...
        full = isNaN(parseFloat($(".full_" + pedido).html().trim())) ? 0 : parseFloat($(".full_" + pedido).html()
            .trim());
        half = isNaN(parseFloat($(".half_" + pedido).html().trim())) ? 0 : parseFloat($(".half_" + pedido).html()
            .trim());
        cuarto = isNaN(parseFloat($(".cuarto_" + pedido).html().trim())) ? 0 : parseFloat($(".cuarto_" + pedido).html()
            .trim());
        //sexto = isNaN(parseFloat($(".sexto_"+pedido).html().trim())) ? 0 : parseFloat($(".sexto_"+pedido).html().trim());
        octavo = isNaN(parseFloat($(".octavo_" + pedido).html().trim())) ? 0 : parseFloat($(".octavo_" + pedido).html()
            .trim());
        sb = isNaN(parseFloat($(".sb_" + pedido).html().trim())) ? 0 : parseFloat($(".sb_" + pedido).html()
            .trim());
    ...
    function calcular_piezas(tabla) {
        piezasxcamion = 0;
        piezas_camion = 0;
        $.each($("input.caja"), function(i, j) {
            if (j.value > 0) {
                piezasxcamion += parseInt(j.value);
            }
        });
        if (piezasxcamion > parseInt($("span#piezas_totales").html())) {
            msg =
                "<span style='color: red'><i class='fa fa-exclamation-triangle'></i> El total de las piezas distribuidas en los camiones sobrepasan la cantidad total de piezas de los pedidos</span>";
            $("#btn_guardar_modal_despacho").attr('disabled', true);
        } else if (piezasxcamion < parseInt($("span#piezas_totales").html())) {
            msg =
                "<span style='color: red;'><i class='fa fa-exclamation-triangle'></i> Aún faltan piezas por distribuir</span>";
            $("#btn_guardar_modal_despacho").attr('disabled', true);
        } else {
        ...
        $.each($("table#" + tabla + " input.caja"), function(i, j) {
            if (j.value > 0) piezas_camion += parseInt(j.value);
        });
