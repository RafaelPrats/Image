<script>
    ...
    $(document).on("click", "#pagination_listado_consignatarios .pagination li a", function(e) {
        $.LoadingOverlay("show");
    ...
    function add_consignatario(id_consignatario) {
        ...
        $.get('{{ url('consignatario/add') }}', datos, function(retorno) {
            modal_view('modal_admin_consignatarios', retorno,
                '<i class="fa fa-user-plus" aria-hidden="true"></i> Añadir consignatario', true, false,
                '{{ isPC() ? '80%' : '' }}');
        });
        $.LoadingOverlay('hide');
    }
