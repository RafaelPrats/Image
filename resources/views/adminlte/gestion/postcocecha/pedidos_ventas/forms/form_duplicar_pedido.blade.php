...
<div class="row" >
    ...
    @elseif($tipoPedido == 'STANDING ORDER')
        <div class="col-md-3">
        ...
        <div class="col-md-3">
            <label for="Especificaciones">Desde</label>
            <input  type="date" id="fecha_desde_pedido_fijo" name="fecha_desde_pedido_fijo" class="form-control" required="">
        </div>
        <div class="col-md-3">
            <label for="Especificaciones">Hasta</label>
            <input  type="date" id="fecha_hasta_pedido_fijo" name="fecha_hasta_pedido_fijo" class="form-control" required="">
        </div>
        <div class="col-md-3">
            ...
            <select class="form-control" id="tipo_pedido_fijo" name="tipo_pedido_fijo">
                <option {{$tipoPedido == 'STANDING ORDER' ? 'selected' : ''}} value="STANDING ORDER">STANDING ORDER</option>
                <option {{$tipoPedido == 'OPEN MARKET' ? 'selected' : ''}} value="OPEN MARKET">OPEN MARKET</option>
            </select>
        </div>
    @endif
</div>
    ...
    function store_duplicar_pedido() {
        ...
        if($("#form_fechas").valid()){
            ...
            let datos= {
                _token : '{{csrf_token()}}',
                arrFechas,
                fecha_desde_pedido_fijo: $("#fecha_desde_pedido_fijo").val(),
                fecha_hasta_pedido_fijo: $("#fecha_hasta_pedido_fijo").val(),
                dia_semana: $("#dia_semana").val(),
                id_pedido : '{{$id_pedido}}',
                tipoPedido: typeof $("#tipo_pedido_fijo").val() == 'undefined' ? 'OPEN MARKET' : $("#tipo_pedido_fijo").val()
            }

            if(datos.tipoPedido=='OPEN MARKET'){
                ...
                if(!arrFechas.length){
                    ...
                    for (var i = 0; i < diferenciaDias + 2; i++) {
                        ...
                        var fechas = date.getFullYear() + "/" +((date.getMonth() + 1) < 10 ? '0'+(date.getMonth() + 1) : (date.getMonth() + 1))+ "/" + (date.getDate() < 10 ? ('0'+date.getDate()) : date.getDate() )
            ...
            if(datos.tipoPedido=='STANDING ORDER' && (datos.fecha_desde_pedido_fijo == '' || datos.fecha_desde_pedido_fijo == '' || datos.dia_semana == '')){
                modal_view('modal_duplicar_pedido','<div class="alert alert-danger text-center"><p> Debe seleccionar el intervalo de fechas para duplicar el pedido </p> </div>','<i class="fa fa-times" aria-hidden="true"></i> Estado pedido', true, false, '40%');
                return false;
                $.LoadingOverlay('hide');
            }
