<script>
    ...
    function eliminar_cliente(id, estado) {
        mensaje = {
            title: estado == 'A' ? '<i class="fa fa-fw fa-trash"></i> Desactivar cliente' :
                '<i class="fa fa-fw fa-unlock"></i> Activar cliente',
            mensaje: estado == 'A' ?
                '<div class="alert alert-danger text-center"><i class="fa fa-fw fa-exclamation-triangle"></i> ¿Está seguro de desactivar este cliente?</div>' :
        ...
        modal_quest('modal_delete_cliente', mensaje['mensaje'], mensaje['title'], true, false,
            '{{ isPC() ? '25%' : '' }}',
            function() {
                datos = {
                    _token: '{{ csrf_token() }}',
    ...
    function add_cliente(id_cliente) {
        ...
        $.get('{{ url('clientes/add') }}', datos, function(retorno) {
            modal_view('modal_admin_especificaciones', retorno,
                '<i class="fa fa-user-plus" aria-hidden="true"></i> Añadir cliente', true, false,
                '{{ isPC() ? '90%' : '' }}');
    ...
    function store_cliente() {
        console.log($("input[name='impresion_etiquetas']:checked").val())
        if ($('#form_add_cliente').valid()) {
            ...
            datos = {
                ...
                impresion: $("input[name='impresion_etiquetas']:checked").val(),
                impresion_nombre: $("input[name='impimir_nombre_etiqueta']:checked").val()
            };
    ...
    function form_importar_clientes() {

        $.LoadingOverlay('show');
        $.get('{{ url('clientes/form_importar_clientes') }}', {}, function(retorno) {
            modal_form('modal_file_importar_cliente', retorno,
                '<i class="fa fa-file-excel-o" aria-hidden="true"></i> Cargar archivo ', true, false,
                '{{ isPC() ? '40%' : '' }}',
                function() {
                    importar_cliente();
                });
    ...
    function importar_cliente() {
        ...
        $.LoadingOverlay('show');
        if ($("#form_importar_clientes").valid()) {
            var formData = new FormData($("#form_importar_clientes")[0]);
            formData.append('_token', '{{ csrf_token() }}');
            $.ajax({
                url: '{{ url('clientes/importar_clientes') }}',
                type: 'POST',
                data: formData,
                cache: false,
                contentType: false,
                ...
                success: function(retorno) {
                    $.LoadingOverlay('hide');
                    modal_view('modal_view_importar_clientes', retorno,
                        '<i class="fa fa-fw fa-table"></i> Importar clientes', true, false,
                        '{{ isPC() ? '50%' : '' }}');
                    buscar_listado();
    ...
    function descarga_fomato_carga_clientes() {

        $.ajax({
            type: "POST",
            dataType: "html",
            contentType: "application/x-www-form-urlencoded",
            url: '{{ url('clientes/formato_carga_clientes') }}',
            data: {
                _token: '{{ csrf_token() }}'
            },
            success: function(data) {
                ...
                $("body").append($a);
                $a.attr("download", "Carga masiva de clientes.xlsx");
                $a[0].click();
                $a.remove();
                $.LoadingOverlay('hide');
            }
