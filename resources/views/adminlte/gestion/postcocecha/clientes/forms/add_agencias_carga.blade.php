                            ...
                            @if (es_server())
                                <th class="text-center" style="border-color: #9d9d9d;width: 10%;">
                                    <button type="button" class="btn btn-xs btn-default" title="Añadir campo"
                                        onclick="cargar_opcion('campos_agencia_carga','','clientes/ver_agencias_carga','add')">
                                        <i class="fa fa-fw fa-plus"></i>
                                    </button>
                                    <button type="button" class="btn btn-xs btn-default" title="Crear agencia de carga"
                                        onclick="create_agencia_carga('','{{ csrf_token() }}','{{ $dataCliente->id_cliente }}')">
                    ...
                    <tbody id="campos_agencia_carga">
                        @if (isset($dataClienteAgenciasCarga) && count($dataClienteAgenciasCarga) > 0)
                            @foreach ($dataClienteAgenciasCarga as $key => $clienteAgenciasCarga)
                                <tr id="tr_select_agencias_carga_{{ $key + 1 }}">
                                    <td>
                                        <div class="">
                                            <select id="select_agencia_carga_{{ $key + 1 }}"
                                                {!! $clienteAgenciasCarga->estado == 0 ? 'disabled' : '' !!} name="select_agencia_carga_{{ $key + 1 }}"
                                                ...
                                                @foreach ($dataAgenciaCarga as $agenciaCarga)
                                                    @php
                                                        $selected = '';
                                                        if (
                                                            $agenciaCarga->id_agencia_carga ===
                                                            $clienteAgenciasCarga->id_agencia_carga
                                                        ) {
                                                            $selected = 'selected=selected';
                                                        }
                                                    @endphp
                                                    <option {{ $selected }}
                                                        value="{{ $agenciaCarga->id_agencia_carga }}">
                                                        {{ $agenciaCarga->nombre }}</option>
                                                @endforeach
                                            </select>
                                            <input type="hidden" id="id_select_agencia_carga_{{ $key + 1 }}"
                                                value="{{ $clienteAgenciasCarga->id_cliente_agencia_carga }}">
                                        </div>
                                            ...
                                            <div class="col-md-12">
                                                <div class="row">
                                                    <div class="col-md-4">
                                                        ...
                                                        <input type="text"
                                                            class="form-control contacto_cliente_agencia_carga"
                                                            value="{{ isset($clienteAgenciasCarga->contacto_cliente_agencia_carga[0]) ? $clienteAgenciasCarga->contacto_cliente_agencia_carga[0]->contacto : '' }}"
                                                            required>
                                                    </div>
                                                    <div class="col-md-4">
                                                        <label> Correo</label>
                                                        <input type="email"
                                                            class="form-control correo_cliente_agencia_carga"
                                                            value="{{ isset($clienteAgenciasCarga->contacto_cliente_agencia_carga[0]) ? $clienteAgenciasCarga->contacto_cliente_agencia_carga[0]->correo : '' }}"
                                                            required>
                                                    </div>
                                                    <div class="col-md-4">
                                    ...
                                    @if (es_server())
                                        <td class="text-center" style="vertical-align: middle">
                                            <button type="button" class="btn btn-xs btn-danger"
                                                title="{!! $clienteAgenciasCarga->estado == 1 ? 'Desactivar' : 'Activar' !!} campo"
                        ...
                        @else
                            <tr id="tr_select_agencias_carga_1">
                                <td>
                                    <div class="">
                                        <select id="select_agencia_carga_1" name="select_agencia_carga_1"
                                            class="form-control" required>
                                            <option disabled selected>Seleccione</option>
                                            @foreach ($dataAgenciaCarga as $agenciaCarga)
                                                <option value="{{ $agenciaCarga->id_agencia_carga }}">
                                                    {{ $agenciaCarga->nombre }}</option>
                                            @endforeach
                                        </select>
            ...
            @if (es_server())
                <div class="text-center">
                    <button class="btn btn-success" type="button" id="btn_guardar_modal_add_cliente"
                        onclick="store_agencias('{{ $dataCliente->id_cliente }}')"><span
                            class="bootstrap-dialog-button-icon fa fa-fw fa-save"></span>Guardar</button>
                </div>
            @endif
        </div>
    </div>
