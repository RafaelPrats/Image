...
<input type="hidden" id="pos_fecha" value="{{ $pos_fecha }}">
...
<script>
    ...
    function store_distribucion() {
        ...
        post_jquery_m('{{ url('postcosecha/store_distribucion') }}', datos, function() {
            cerrar_modals();
            pos_comb = $('#pos_comb_dist').val();
            pos_fecha = $('#pos_fecha').val();
            distribuir_trabajo(pos_comb, pos_fecha);
            listar_reporte();
        });
    }
    ...
    function delete_distribucion(id) {
        ...
        modal_quest('modal_quest_delete_distribucion', '<div class="alert alert-info text-center">' +
            ...
            '{{ isPC() ? '35%' : '' }}',
            function() {
                post_jquery_m('{{ url('postcosecha/delete_distribucion') }}', datos, function() {
                    $('#tr_dist_' + id).remove();
                    listar_reporte();
                });
            });
