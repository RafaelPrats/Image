        ...
        <td>
            <div class="form-check text-left">
                ...
                <select class="form-check-input" id="select_paginas"
                    onchange="cerrar_modals(); distribuir_mixtos('{{ $cliente->id_cliente }}', '{{ $longitud }}')">
...
<div style="overflow-x: scroll; overflow-y: scroll; max-height: 500px;">
    <table class="table-stripped table-bordered" style="width: 100%; border: 1px solid #9d9d9d">
        <tr>
            ...
            @foreach ($pedidos_mixtos as $pos => $p)
                <th class="text-center bg-yura_dark" colspan="7" style="border-left: 2px solid">
                    ...
                    <input type="hidden" id="guia_id_proyecto_{{ $pos }}" value="{{ $p->id_proyecto }}">
                    <input type="hidden" id="guia_id_caja_proyecto_{{ $pos }}"
                        value="{{ $p->id_caja_proyecto }}">
                    <input type="hidden" id="guia_id_detalle_caja_proyecto_{{ $pos }}"
                        value="{{ $p->id_detalle_caja_proyecto }}">
                    <input type="hidden" id="guia_id_cliente_{{ $pos }}" value="{{ $p->id_cliente }}">
                    <input type="hidden" id="guia_nombre_cliente_{{ $pos }}" value="{{ $cliente->nombre }}">
                    <input type="hidden" id="guia_longitud_ramo_{{ $pos }}" value="{{ $p->longitud_ramo }}">
                    <input type="hidden" id="guia_tallos_x_ramo_{{ $pos }}" value="{{ $p->tallos_x_ramo }}">
                    <input type="hidden" id="guia_ramos_x_caja_{{ $pos }}" value="{{ $p->ramos_x_caja }}">
                    <input type="hidden" id="guia_piezas_{{ $pos }}" value="{{ $p->piezas }}">
                    {{ $cliente->nombre }}
                    <sup>{{ $p->longitud_ramo }}cm</sup>
                </th>
                @php
                    $ramos_mixtos += $p->piezas * $p->ramos_x_caja * $p->tallos_x_ramo;
                    $totales[] = [
                        'porcentaje' => 0,
                        'tallos' => 0,
                        'ramos' => 0,
        ...
        <tr>
            @foreach ($pedidos_mixtos as $pos => $p)
                @php
                    $tallos = $p->piezas * $p->ramos_x_caja * $p->tallos_x_ramo;
                @endphp
        ...
        @foreach ($listado as $pos_v => $item)
            ...
            @endphp
            <tr>
                <input type="hidden" class="ids_variedad" value="{{ $item['var']->id_variedad }}">
                ...
                <th class="text-center {{ $item['consolidado'] > 0 ? 'text-color_yura' : 'text-color_yura_danger' }}"
                    ...
                    <input type="number" readonly style="width: 100%; background-color: #efefef" class="text-center"
                        ...
                        id="consolidado_variedad_{{ $item['var']->id_variedad }}">
                </th>
                <th class="text-center {{ $item['saldo'] > 0 ? 'text-color_yura' : 'text-color_yura_danger' }}"
                    style="border-color: #9d9d9d">
                    <input type="number" readonly style="width: 100%; background-color: #efefef" class="text-center"
                        value="{{ $item['saldo'] }}" id="saldo_variedad_{{ $item['var']->id_variedad }}">
                </th>
                @foreach ($item['valores'] as $pos => $val)
                    @php
                        ...
                        if ($item['backups'][$pos]['model'] != '') {
                            ...
                            $back_tallos_x_ramo = $item['backups'][$pos]['model']->tallos_x_ramo;
                        } else {
                            $back_porcentaje = null;
                            $back_tallos = null;
                            $back_ramos = null;
                            $back_piezas = null;
                            $back_tallos_x_ramo = null;
                        }
                    @endphp
                    <th class="text-center padding_lateral_5" style="border-color: #9d9d9d; border-left: 2px solid"
                        id="porcentaje_variedad_{{ $item['var']->id_variedad }}_{{ $pos }}">
                        {{ $porcentaje }}%
                    </th>
                    <td class="text-center" style="border-color: #9d9d9d">
                        <input type="number" class="text-center" style="width: 100%" value="{{ $tallos }}"
                            readonly id="tallos_variedad_{{ $item['var']->id_variedad }}_{{ $pos }}">
                    </td>
                    ...
                    <th class="text-center" style="border-color: #9d9d9d" title="Ramos totales">
                        <input type="number" disabled class="text-center" style="width: 100%"
                            ...
                            id="total_ramos_variedad_{{ $item['var']->id_variedad }}_{{ $pos }}">
                    </th>
                    <th class="text-center" style="border-color: #9d9d9d" title="Anterior Distribucion">
                        <input type="number" disabled style="width: 100%"
                            ...
                            id="back_ramos_variedad_{{ $item['var']->id_variedad }}_{{ $pos }}"
                            class="text-center marcar_nuevo_{{ $pos }}">
                    </th>
                    <th class="text-center" style="border-color: #9d9d9d" title="Ramos totales">
                        <input type="number" disabled style="width: 100%"
                            value="{{ $item['backups'][$pos]['model'] != null ? $back_ramos * $back_piezas : 0 }}"
                            id="total_back_ramos_variedad_{{ $item['var']->id_variedad }}_{{ $pos }}"
                            class="text-center marcar_nuevo_{{ $pos }}">
                    </th>
                @endforeach
                @php
                ...
                <th class="text-center {{ $tallos_var > 0 ? 'text-color_yura' : 'text-color_yura_danger' }}"
                    ...
                    <input type="number" readonly style="width: 100%; background-color: #efefef"
                        ...
                        id="tallos_variedad_{{ $item['var']->id_variedad }}">
                </th>
            </tr>
        @endforeach
        <!-- TOTALES -->
...
<input type="hidden" id="cliente_seleccionado" value="{{ $cliente->id_cliente }}">
        ...
        if (guia_ramos_mixtos == tallos_total) {
            ...
            for (i = 0; i < ids_variedad.length; i++) {
                ...
                for (x = 0; x < pos.length; x++) {
                    ...
                    guia_id_detalle_caja_proyecto = parseInt($('#guia_id_detalle_caja_proyecto_' + p)
                        .val());
                    guia_id_proyecto = parseInt($('#guia_id_proyecto_' + p).val());
                    guia_id_caja_proyecto = parseInt($('#guia_id_caja_proyecto_' + p).val());
                    ...
                    guia_ramos_x_caja = parseInt($('#guia_ramos_x_caja_' + p).val());
                    guia_tallos_x_ramo = parseInt($('#guia_tallos_x_ramo_' + p).val());
                    guia_piezas = parseInt($('#guia_piezas_' + p).val());
                    data.push({
                        var: id_var,
                        ramos: ramos,
                        ...
                        guia_id_detalle_caja_proyecto: guia_id_detalle_caja_proyecto,
                        guia_id_proyecto: guia_id_proyecto,
                        guia_id_caja_proyecto: guia_id_caja_proyecto,
                        ...
                        guia_tallos_x_ramo: guia_tallos_x_ramo,
            ...
            post_jquery_m('{{ url('distribucion_mixtos/store_distribucion') }}', datos, function(retorno) {
                cerrar_modals();
                @if ($cantidad_paginas > 1)
    ...
    function eliminar_distribuciones() {
        ...
        modal_quest('modal_update_factor_conversion', mensaje['mensaje'], mensaje['title'], true, false,
            ...
            function() {
                ...
                post_jquery_m('{{ url('distribucion_mixtos/eliminar_distribuciones') }}', datos, function(
                    retorno) {
                    cerrar_modals();
                    distribuir_mixtos(datos['cliente'], datos['longitud']);
                    listar_reporte();
