...
<div style="overflow-y: scroll; max-height: 500px">
    @php
        $pos_dia = transformDiaPhp(date('w', strtotime($pedido->fecha)));
    @endphp
    <table class="table-bordered" style="border: 1px solid #9d9d9d; width: 100%">
        ...
        @php
            $fecha_actual = $pedido->fecha;
        @endphp
        @foreach ($ordenes as $pos => $o)
            @php
                $pos_dia = transformDiaPhp(date('w', strtotime($o->fecha)));
            @endphp
            <tr>
                <th class="text-center" style="border-color: #9d9d9d">
                    <input type="checkbox" id="check_fecha_{{ $o->id_proyecto }}" class="check_fecha"
                        value="{{ $o->id_proyecto }}">
                </th>
                <th class="text-center" style="border-color: #9d9d9d">
                    <label for="check_fecha_{{ $o->id_proyecto }}" class="mouse-hand">
                        {{ getDias(TP_ABREVIADO, FR_ARREGLO)[$pos_dia] }}
                        {{ convertDateToText($o->fecha) }}
                    </label>
                    @if ($pos > 0)
                        <sup>+{{ difFechas($o->fecha, $fecha_actual)->days }}</sup>
                    @endif
                </th>
                <th class="text-center" style="border-color: #9d9d9d">
                    <select id="select_fecha_{{ $o->id_proyecto }}" style="width: 100%; height: 28px;">
                        <option value="0" {{ $pos_dia == 0 ? 'selected' : '' }}>Lunes</option>
                        <option value="1" {{ $pos_dia == 1 ? 'selected' : '' }}>Martes</option>
                        <option value="2" {{ $pos_dia == 2 ? 'selected' : '' }}>Miercoles</option>
                        <option value="3" {{ $pos_dia == 3 ? 'selected' : '' }}>Jueves</option>
...
<div class="text-center" style="margin-top: 5px">
    <button type="button" class="btn btn-yura_primary" onclick="store_mover_fechas('{{ $pedido->id_proyecto }}')">
...
<script>
    ...
    function store_mover_fechas(pedido) {
        ...
        post_jquery_m('{{ url('proyectos/store_mover_fechas') }}', datos, function(retorno) {
            cerrar_modals();
