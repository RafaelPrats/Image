<table class="table-responsive table-striped table-bordered table-hover tabla_master" width="100%" id="table_producto">
    ...
    <tr class="fila_fija">
        ...
        <th class="text-center" style="border-color: #9d9d9d; width: 60px">Precio</th>
        @if(es_local())
    ...
    @foreach($productos as $a)
        <tr id="tr_producto_{{$a->id_producto}}" class="{{$a->estado == 0 ? 'error' : ''}}">
            ...
            <td style="border-color: #9d9d9d" class="text-center">
                <input type="number" maxlength="11" id="n_precio_{{$a->id_producto}}" name="n_precio_{{$a->id_producto}}"
                       value="{{$a->precio}}" style="width: 100%" class="text-center">
            </td>
            @if(es_local())
...
<script>
    ...
    function add_producto() {
        ...
        $('#table_producto').append('<tr id="new_tr_producto_' + cant_forms_producto + '">' +
            ...
            '</td>' +
            '<td class="text-center">' +
            '<input type="number" maxlength="11" id="new_precio_' + cant_forms_producto + '" name="new_precio_' + cant_forms_producto + '" style="width: 100%" class="text-center">' +
            '</td>' +
            '<td class="text-center">' +
