<script>
    ...
    function buscar_listado() {
        $.LoadingOverlay('show');
        datos = {
            busqueda: $('#input_busqueda_dae').val().trim(),
            anno: $('#input_busqueda_anno').val(),
            mes: $('#input_busqueda_mes').val(),
        };
    ...
    function add_codigo_dae() {
        ...
        $.get('{{ url('codigo_dae/seleccionar_pais') }}', datos, function(retorno) {
            modal_form('modal_add_marca', retorno, '<i class="fa fa-fw fa-plus"></i> Seleccionar paises', true,
                false, '{{ isPC() ? '80%' : '' }}',
                function() {
                    exel_paises();
                    $.LoadingOverlay('hide');
        ...
        if ($("#" + id).is(':checked')) {
            ...
            $.get('{{ url('codigo_dae/pais') }}', datos, function(retorno) {
                $("#paises_selected").append("" +
                    "<a href='javascript:void(0)' id='" + retorno.codigo +
                    "' style='padding:5px 15px' class='list-group-item list-group-item-action'>" +
                    "<input type='hidden' id='codigo_pais_selected' name='codigo_pais_selected' value='" +
                    retorno.codigo + "'> " +
                    "" + retorno.nombre + "" +
                    "</a>");
            });
    ...
    function exel_paises() {
        if ($('input[name="codigo_pais_selected"]').length === 0) {
            modal_view('modal_view_msg_pais_selected',
                ...
                '<i class="fa fa-times" aria-hidden="true"></i> Paises seleccionados', true, false,
                '{{ isPC() ? '50%' : '' }}');
            return false;
        }
        $.LoadingOverlay('show');
        ...
        $.LoadingOverlay('show');
        window.open('{{ url('codigo_dae/exportar_paises') }}?arreglo=' + arreglo, '_blank');
        $.LoadingOverlay('hide');
    }
    ...
    function subir_codigo_dae() {
        ...
        $.get('{{ url('codigo_dae/form_file_codigo_dae') }}', datos, function(retorno) {
            modal_form('modal_upload_file_codigo_dae', retorno,
                '<i class="fa fa-file-excel-o"></i> Seleccionar el archivo Excel', true, false,
                '{{ isPC() ? '40%' : '' }}',
                function() {
                    importar_codigo_dae();
                    $.LoadingOverlay('hide');
        ...
        if ($("#form_add_codigo_dae").valid()) {
            ...
            $.ajax({
                ...
                success: function(retorno) {
                    modal_view('modal_view_codigo_dae', retorno,
                        '<i class="fa fa-fw fa-table"></i> Codigo DAE', true, false,
                        '{{ isPC() ? '50%' : '' }}');
                    buscar_listado();
                    $.LoadingOverlay('hide');
                },
    ...
    function desactivar_codigo(id_codigo) {
        modal_quest('modal_update_estado_codigo_dae',
            '<div class="alert alert-danger text-center"><i class="fa fa-fw fa-exclamation-triangle"></i> ¿Está seguro de desactivar este código DAE? </div>',
            '<i class="fa fa-fw fa-trash"></i> Desactivar código DAE', true, false, '{{ isPC() ? '40%' : '' }}',
            function() {
                $.LoadingOverlay('show');
                var datos = {
                    _token: '{{ csrf_token() }}',
