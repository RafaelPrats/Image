                        ...
                        <table class="table-bordered table-stripped" style="border: 1px solid #9d9d9d; width: 100%;">
                            ...
                            @foreach ($listado_clientes as $p)
                                @php
                                    ...
                                    if ($p['distribuido'] == $p['mixtos_x_cliente']) {
                                ...
                                <tr title="{{ $title_tr }}">
                                    ...
                                    <th class="text-center padding_lateral_5 {{ $clase_text_color }}"
                                        ...
                                        {{ number_format($p['mixtos_x_cliente']) }}
                                        <input type="hidden"
                                            id="tallos_pedidos_mixtos_{{ $p['id_cliente'] }}_{{ $p['longitud_ramo'] }}"
                                            value="{{ $p['mixtos_x_cliente'] }}">
                                    </th>
                                    <th class="text-center padding_lateral_5 {{ $clase_text_color }}"
                ...
                @if (es_server())
                    ...
                    <div class="btn-group">
                        ...
                        <button type="button" class="btn btn-xs btn-yura_danger"
                            onclick="$('.input_dist').val(0); calcular_totales_distribucion_diaria()"
                            title="Todo en 0">
...
<script>
    ...
    function distribuir_mixtos(cliente, longitud) {
        ...
        get_jquery('{{ url('distribucion_cosecha/distribuir_mixtos') }}', datos, function(retorno) {
    ...
    function store_distribucion_mixtos_diaria() {
        ...
        post_jquery_m('{{ url('distribucion_cosecha/store_distribucion_mixtos_diaria') }}', datos, function() {
            listar_reporte();
        });
    }
    ...
    function duplicar_distribucion() {
        ...
        post_jquery_m('{{ url('distribucion_cosecha/duplicar_distribucion') }}', datos, function() {
            //listar_reporte();
        });
    }
</script>
