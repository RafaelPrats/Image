<script>
    vista_actual = 'caja_presentacion';
    buscar_empaques();
    ...
    function add_empaque(id_empaque) {
        ...
        $.get('{{ url('caja_presentacion/add_empaque') }}', datos, function(retorno) {
            modal_form('modal_add_empaque', retorno,
                '<i class="fa fa-pencil" aria-hidden="true"></i> Editar nombre del empaque', true, false,
                '{{ isPC() ? '70%' : '' }}',
                function() {
    ...
    function store_empaque(id_empaque) {
        datos = {
            ...
            conversion: $("#conversion_empaque").val(),
            peso: $("#peso_empaque").val(),
            tipo: $("#tipo").val()
        };
        $.LoadingOverlay('show');
        $.get('{{ url('caja_presentacion/store_empaque') }}', datos, function(retorno) {
            modal_view('modal_empaque', retorno, '<i class="fa fa-fw fa-gift"></i> Mensaje empaque', true,
                false, '{{ isPC() ? '40%' : '' }}');
            buscar_empaques();
            cerrar_modals();
        }).always(function() {
    ...
    function form_add_detalle_empaque() {
        ...
        $.get('{{ url('caja_presentacion/form_file_detalle_empaque') }}', {}, function(retorno) {
            modal_form('modal_file_detalle_empaque', retorno,
                '<i class="fa fa-file-excel-o" aria-hidden="true"></i> Archivo detalles empaque', true,
                false, '{{ isPC() ? '40%' : '' }}',
                function() {
        ...
        if ($("#form_add_detalle_empaque").valid()) {
            ...
            $.ajax({
                ...
                success: function(retorno) {
                    $.LoadingOverlay('hide');
                    modal_view('modal_view_detalle_empaque', retorno,
                        '<i class="fa fa-fw fa-table"></i> Detalles empaque', true, false,
                        '{{ isPC() ? '50%' : '' }}');
                    // buscar_listado();
    ...
    function detalle_empaque(id_empaque) {
        ...
        $.get('{{ url('caja_presentacion/detalle_empaque') }}', datos, function(retorno) {
            modal_view('modal_detalle_empaque', retorno,
                '<i class="fa fa-list" aria-hidden="true"></i> Detalles empaque <span id="span_nombre_empaque" style="font-weight:900"></span>',
                true, false, '{{ isPC() ? '60%' : '' }}');
    ...
    function store_detalle_empque() {
        ...
        $.get('{{ url('caja_presentacion/store_detalle_empaque') }}', datos, function(retorno) {
            modal_view('modal_empaque', retorno, '<i class="fa fa-fw fa-gift"></i> Mensaje empaque </span>',
                true, false, '{{ isPC() ? '40%' : '' }}');
            buscar_empaques();
            cerrar_modals();
        }).always(function() {
    ...
    function delete_detalle_empaque(id_detalle_empaque, id_empaque) {
        modal_quest('modal_message_facturar_envios',
            ...
            '<i class="fa fa-list-alt" aria-hidden="true"></i> Mensaje', true, false, '{{ isPC() ? '60%' : '' }}',
            function() {
                datos = {
                ...
                $.get('{{ url('caja_presentacion/delete_detalle_empaque') }}', datos, function(retorno) {
                    modal_view('modal_detalle_empaque', retorno,
                        '<i class="fa fa-fw fa-gift"></i> Mensaje detalle empaque', true, false,
                        '{{ isPC() ? '40%' : '' }}');
                    cerrar_modals();
