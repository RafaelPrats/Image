<div style="overflow-y: scroll; max-height: 450px">
    ...
    <table class="table-striped table-bordered" style="width: 100%; border: 1px solid #9d9d9d; border-radius: 18px 18px 0 0"
        ...
        <tr id="tr_fija_top_0">
            ...
            <th class="text-center th_yura_green" title="Repeticiones">
                Rep.
            </th>
            <th class="text-center th_yura_green">
                Veces x Semana
            </th>
            <th class="text-center th_yura_green" style="width: 80px;" title="Frecuencia">
                Frec.
            </th>
            ...
            @endif
            <th class="text-center th_yura_green" style="width: 80px;" title="Continua">
                Cont.
            </th>
            <th class="text-center th_yura_green" style="width: 80px;">
                Tipo
            ...
            <th class="text-center th_yura_green" style="border-radius: 0 18px 0 0;">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        <li><a class="dropdown-item" href="javascript:void(0)" onclick="add_aplicacion()">Aplicación</a></li>
        ...
        @foreach($listado as $item)
            <tr id="tr_app_{{$item->id_aplicacion}}">
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="text" id="nombre_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center"
                           value="{{$item->nombre}}" placeholder="Nombre">
                    <span class="hidden">{{$item->nombre}}</span>
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="dia_ini_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center mouse-hand"
                           value="{{$item->dia_ini}}" onkeyup="calcular_semana_ini('{{$item->id_aplicacion}}')"
                ...
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="semana_ini_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center mouse-hand"
                           value="{{$item->semana_ini}}" onchange="$('#dia_ini_app_{{$item->id_aplicacion}}').val(0)"
                           title="Doble click para parametrizar" data-placement="bottom" data-toggle="tooltip"
                           ondblclick="parametrizar_app('semana_ini', '{{$item->id_aplicacion}}')">
                    <span class="hidden">{{$item->semana_ini}}</span>
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="repeticiones_app_{{$item->id_aplicacion}}" style="width: 100%;" class="text-center mouse-hand"
                           value="{{$item->repeticiones}}" title="Doble click para parametrizar" data-placement="bottom"
                           data-toggle="tooltip" min="1"
                           ondblclick="parametrizar_app('repeticiones', '{{$item->id_aplicacion}}')">
                    <span class="hidden">{{$item->repeticiones}}</span>
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="veces_x_semana_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center mouse-hand"
                           value="{{$item->veces_x_semana}}" title="Doble click para parametrizar" data-placement="bottom"
                           data-toggle="tooltip" min="1"
                           ondblclick="parametrizar_app('veces_x_semana', '{{$item->id_aplicacion}}')">
                    <span class="hidden">{{$item->veces_x_semana}}</span>
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <input type="number" id="frecuencia_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center" min="0"
                           value="{{$item->frecuencia}}">
                    <span class="hidden">{{$item->frecuencia}}</span>
                </td>
                <td class="text-center" style="border-color: #9d9d9d">
                    <select id="poda_siembra_app_{{$item->id_aplicacion}}" style="width: 100%">
                        <option value="T" {{$item->poda_siembra == 'T' ? 'selected' : ''}}>Podas y Siembras</option>
                        <option value="P" {{$item->poda_siembra == 'P' ? 'selected' : ''}}>Podas</option>
                        <option value="S" {{$item->poda_siembra == 'S' ? 'selected' : ''}}>Siembras</option>
                    </select>
                    <span class="hidden">{{$item->poda_siembra}}</span>
                </td>
                @if($tipo == 'S')
                    <td class="text-center" style="border-color: #9d9d9d">
                        <input type="number" id="litro_x_cama_app_{{$item->id_aplicacion}}" style="width: 100%" class="text-center" min="1"
                               value="{{$item->litro_x_cama}}">
                        <span class="hidden">{{$item->litro_x_cama}}</span>
                    </td>
                @endif
                <td class="text-center" style="border-color: #9d9d9d">
                    <select id="continua_app_{{$item->id_aplicacion}}" style="width: 100%">
                ...
                <td class="text-center" style="border-color: #9d9d9d">
                    <select id="tipo_app_{{$item->id_aplicacion}}" style="width: 100%">
                ...
                <td class="text-center" style="border-color: #9d9d9d">
                    <select id="app_matriz_{{$item->id_aplicacion}}" style="width: 100%">
                ...
                <td class="text-center" style="border-color: #9d9d9d">
                    <div class="btn-group">
                        ...
                        <ul class="dropdown-menu dropdown-menu-right">
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" onclick="update_app('{{$item->id_aplicacion}}')">
                                    <i class="fa fa-fw fa-edit"></i> Editar Aplicación
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" onclick="mezclas_app('{{$item->id_aplicacion_matriz}}')">
                                    <i class="fa fa-fw fa-flask"></i> Mezclas de la Aplicación
                                </a>
                            </li>
                            <li>
                                <a class="dropdown-item" href="javascript:void(0)" onclick="variedades_app('{{$item->id_aplicacion}}')">
...
<script>
    ...
    function add_matriz() {
        get_jquery('{{url('aplicaciones_campo/add_matriz')}}', {}, function (retorno) {
            modal_view('modal-view_add_matriz', retorno, '<i class="fa fa-fw fa-plus"></i> Labores matriz', true, false, '50%');
        });
    }
</script>