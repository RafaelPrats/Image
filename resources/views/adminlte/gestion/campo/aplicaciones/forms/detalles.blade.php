...
<table class="table-bordered table-striped" style="width: 100%; border: 1px solid #9d9d9d; border-radius: 18px 18px 0 0"
    ...
    <tr>
        <th class="text-center th_yura_green" style="border-radius: 18px 0 0 0" colspan="2">
            Insumo
        </th>
        <th class="text-center th_yura_green" style="width: 50%" colspan="2">
            Mano de Obra
        </th>
    ...
    @foreach($m->getModelDetalles() as $det)
        ...
        <tr id="tr_det_app_{{$det->id_detalle_aplicacion}}">
            ...
            <td class="text-center" style="border-color: #9d9d9d">
                @if($det->id_producto != '')
                    @if($dosis == '')
                        <a href="javascript:void(0)" onclick="parametrizar_det('{{$det->id_detalle_aplicacion}}')">
                            ...
                        </a>
                    @else
                        {{$dosis}}{{$um}}
        ...
        $.post('{{url('aplicaciones_campo/store_detalle_app')}}', datos, function (retorno) {
            if (retorno.success) {
                $('#table_detalles_' + mezcla).append('<tr id="tr_det_app_' + retorno.model.id_det + '">' +
                    ...
                    '</td>' +
                    '<td class="text-center" style="border-color: #9d9d9d">...</td>' +
                    '<td class="text-center" style="border-color: #9d9d9d">' +
                    retorno.model.mo +
                    '</td>' +
                    '<td class="text-center" style="border-color: #9d9d9d">...</td>' +
