        ...
        @foreach($listado as $i => $item)
            @php
                ...
                if($item['ciclo']->poda_siembra == 'S')
                    $litros = $labor != '' ? $labor->litro_x_cama : $mezcla->litro_x_cama;
                else
                    $litros = $labor != '' ? $labor->litro_x_cama : $mezcla->litro_x_cama_poda;
                ...
                if ($labor == ''){
                    ...
                    if ($posicion != false){
                        if($item['ciclo']->poda_siembra == 'S')
                            $litros = explode('-', $mezcla->litros_x_repeticiones)[$posicion];
                        else
                            $litros = explode('-', $mezcla->litros_x_repeticiones_poda)[$posicion];
                    }
                }
            @endphp
            <tr id="m-tr_{{$i}}">
                ...
                <input type="hidden" id="m-check_luz_{{$i}}" value="{{$item['data']['check_luz']}}">
                @foreach($productos as $p)
                    @php
                        $dosis = '';
                        $unidad_medida = '';
    ...
    function store_mezclas(mezcla) {
        ...
        for (i = 0; i < array_pos_i.length; i++) {
            ...
            data.push({
                ...
                check_luz: $('#m-check_luz_' + pos_i).val(),
                productos: productos,
