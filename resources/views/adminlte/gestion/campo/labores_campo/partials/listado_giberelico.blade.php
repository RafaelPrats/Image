<div style="overflow-y: scroll; overflow-x: scroll; height: 450px">
    <table class="table-bordered table-striped" style="width: 100%; border: 1px solid #9d9d9d" id="table_labores">
            ...
            <th class="text-center th_yura_green">
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu dropdown-menu-right">
                        ...
                        @endif
                        <li>
                            <a class="dropdown-item" href="javascript:void(0)" onclick="$('.th_detalles').toggleClass('hidden')">
            ...
            @for($i = 0; $i < $count; $i++)
                @php
                    ...
                    if ($labor != ''){
                        ...
                        $litro_x_cama = $aplicacion->litro_x_cama;
                        $lastLabor = $item['ciclo']->getLastLaborByFecha($aplicacion->id_aplicacion, $semana_req->fecha_final);
                        for($y = 0; $y < count($rangos); $y++){
                            if (($dias_ciclo_ini_sem >= $rangos[$y]['desde'] && $dias_ciclo_ini_sem <= $rangos[$y]['hasta']) ||
                                (($dias_ciclo_fin_sem >= $rangos[$y]['desde'] && $dias_ciclo_fin_sem <= $rangos[$y]['hasta']))){
                                if (isset($lastLabor)){
                ...
                @if($show)
                    <tr id="tr_ciclo_{{$pos}}">
                        ...
                        <th class="text-center" style="border-color: #9d9d9d; background-color: #e9ecef">
                            <input type="checkbox" id="check_luz_{{$pos}}" class="{{$repeticion == 1 ? '' : 'hidden'}}" checked>
                        </th>
                        ...
                        <td class="text-center" style="border-color: #9d9d9d; background-color: #e9ecef">
                            ...
                            <span class="hidden" id="span_litros_x_cama_{{$pos}}">{{$litro_x_cama}}</span>
                        </td>
                        ...
                        <td class="text-center" style="border-color: #9d9d9d; background-color: #e9ecef">
                            <div class="btn-group">
                                ...
                                @else
                                    ...
                                    <input type="hidden" id="id_aplicacion_campo_{{$pos}}" value="{{$labor->id_aplicacion_campo}}">
                                @endif
                            </div>
                        </td>
                    </tr>
        ...
        <tfoot>
        <tr>
            <th class="th_yura_green" colspan="9" style="padding-left: 10px">
                TOTALES
            </th>
            <th class="text-center th_yura_green">
...
<script>
    ...
    function aplicar_mezclas() {
        ...
        get_jquery('{{url('ingreso_labores/aplicar_mezclas')}}', datos, function (retorno) {
            modal_view('modal_view-aplicar_mezclas', retorno, '<i class="fa fa-fw fa-eye"></i> Aplicar mezclas', true, false, '90%');
        });
    }
