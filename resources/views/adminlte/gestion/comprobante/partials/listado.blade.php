<div id="table_comprobante">
    @if(sizeof($listado)>0)
        <table width="100%" class="table-responsive table-bordered" style="font-size: 0.9em; border-color: #9d9d9d"
            ...
            @foreach($listado as $key => $item)
                @if(count($item->envio->pedido->detalles) == 0)
                    @dd($item->envio->pedido->id_pedido, $item)
                @endif
                ...
                <tr onmouseover="$(this).css('background-color','#add8e6')" onmouseleave="$(this).css('background-color','')"
                    ...
                    @endif
                    <td style="border-color: #9d9d9d" class="text-center">{{$item->nombre_comprobante}}
                        - {{$item->secuencial}}  {{-- {{$item->nombre_comprobante." N# "."001-".{{--getDetallesClaveAcceso($item->clave_acceso, 'PUNTO_ACCESO')."-".getDetallesClaveAcceso($item->clave_acceso, 'SECUENCIAL')}} COMENTADO PARA QUE LA FACTURACION FUNCIONE CON EL VENTURE --}}</td>
                    @if($tipo_comprobante=="01")
                        <td style="border-color: #9d9d9d" class="text-center">
                    ...
                    @if(es_Server())
                        <td style="border-color: #9d9d9d;width:150px" class="text-center">
                            @if($item->estado==5)
                                <a target="_blank" href="{{url('comprobante/comprobante_aprobado_sri',$item->clave_acceso)}}"
                                class="btn btn-info btn-xs" title="Ver factura">
                                ...
                                @if($tipo_comprobante=="01" && getComprobanteRelacionadFactura($item->id_comprobante) == null)
                                    @if(getCantDespacho(getComprobante($item->id_comprobante)->envio->pedido->id_pedido)>0)
                                        ...
                                        <button class="btn btn-success btn-xs" title="Crear Guía de Remisión"
                                                onclick="crear_guia_remision_factura('{{$item->id_comprobante}}')">
                                            <i class="fa fa-file-text-o" aria-hidden="true"></i>
                                        </button>
                                    @endif
                                @endif
                                @if($tipo_comprobante!="06")
                                    ...
                                    <a target="_blank" href="{{url('comprobante/documento_pre_factura',[$item->secuencial,true])}}"
                                    class="btn btn-info btn-xs" title="Ver factura Cliente">
                                        <i class="fa fa-user-circle-o" aria-hidden="true"></i>
                                    </a>

                                    @if($item->esatdo == 1)
                                        <a target="_blank" href="{{url('comprobante/documento_pre_factura',$item->secuencial)}}"
                                        class="btn btn-primary btn-xs" title="Ver factura SRI">
                                            <i class="fa fa-eye" aria-hidden="true"></i>
                                        </a>
                                    @endif
                                    <button class="btn btn-danger btn-xs" title="Anular factura"
                                            onclick="anular_factura('{{$item->id_comprobante}}')">
                                        <i class="fa fa-times"></i>
                                    </button>
                                @else
                                    <a target="_blank" href="{{url('comprobante/guia_remision',$item->secuencial)}}" class="btn btn-primary btn-xs"
                            ...
                            @if($item->estado == 1)
                                @if($item->integrado)
                                    <button class="btn btn-warning btn-xs" title="Desvincular del venture"
                                            onclick="update_integrado('{{$item->id_comprobante}}')">
                                        <i class="fa fa-ban"></i>
                                    </button>
                                @else
                                    @if(!$item->ficticio)
                                        <button class="btn btn-default btn-xs" title="Integrar factura"
                                                onclick="integrar_factura_venture('{{$item->id_comprobante}}')">
                            ...
                            @elseif($item->estado == 0)
                                <button class="btn btn-default btn-xs">
                                    <input type="checkbox" id="firmar_{{$key+1}}" name="firmar" value="{{$item->id_comprobante}}"
            ...
            <tr>
                <td colspan="3" style="border-color: #9d9d9d"></td>
                <td class="text-right" style="border-color: #9d9d9d"><b>TOTALES: </b></td>
                <td class="text-center" style="border-color: #9d9d9d"><b>${{number_format($monto_total,2,".","")}}</b></td>
                <td class="text-center" style="border-color: #9d9d9d"><b>{{$total_cajas_full}}</b></td>
        ...
        @if($item->estado !=5 && $item->estado != 3 && $item->estado != 4)
            @if($item->estado == 0)
                <div class="text-center">
                    <button class="btn btn-yura_primary" onclick="firmar_comprobante()">
            ...
            @elseif($item->estado == 1)
                <div class="text-center">
                    ...
                    <button class="btn btn-yura_primary" title="Descargar txt" onclick="txt_venture()">
                        <i class="fa fa-download"></i>
                        Descargar archivo txt
                    </button>
                    @if(es_server())
                    <button class="btn btn-yura_default" onclick="actualizar_comprobante()">
