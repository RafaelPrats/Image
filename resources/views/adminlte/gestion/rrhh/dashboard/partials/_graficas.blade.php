<div class="nav-tabs-custom" style="cursor: move;">
    <ul class="nav nav-pills nav-justified">
        <li class="active">
            <a href="#esqueje_x_planta-chart" class="border-radius_18" data-toggle="tab" aria-expanded="true">Esqueje x Planta</a>
        </li>
        <li class="">
            <a href="#costo_x_esqueje-chart" class="border-radius_18" data-toggle="tab" aria-expanded="false">Costo x Esqueje</a>
        </li>
        <li class="">
    ...
    <div class="tab-content no-padding">
        ...
        <div class="chart tab-pane" id="costo_x_esqueje-chart" style="position: relative">
            <canvas id="chart_costo_x_esqueje" width="100%" height="20" style="margin-top: 5px"></canvas>
        </div>
...
<script>
    ...
    construir_char('Costo x Esqueje', 'chart_costo_x_esqueje');
    ...
    function construir_char(label, id) {
        ...
        if (label == 'Costo x Esqueje') {
            @foreach($resumen_propagacion as $pos => $item)
            labels.push('{{$item->semana}}');
            data_list.push('{{count($ids_variedad) > 0 ? round($item->costo_x_esqueje / count($ids_variedad), 3) * 100 : 0}}');
            @endforeach
        }
        ...
        if (label == 'Costo x Planta') {
            ...
            labels.push('{{$item->semana}}');
            data_list.push('{{$cant_validos_cost_x_plta[$pos]->cantidad > 0 ? round(($item->cantidad / $cant_validos_cost_x_plta[$pos]->cantidad) * 100, 4) : 0}}');
            @endforeach
        }
