<script>
    ...
    function buscar_listado() {
        ...
        datos = {
            ...
            estado: $('#estado_agencias_carga').val()
        };
    ...
    $(document).on("click", "#pagination_listado_recepciones .pagination li a", function(e) {
        ...
        url = url.replace('?', '?busqueda=' + $('#busqueda_agencias_carga').val() + '&estado= ' + $(
            '#estado_agencias_carga').val() + '&');
    ...
    function actualizar_agencia_carga(id_agencia_carga, estado_agencia_carga) {
        mensaje = {
            title: estado_agencia_carga == 1 ? '<i class="fa fa-fw fa-trash"></i> Desactivar agencia de carga' :
                '<i class="fa fa-fw fa-unlock"></i> Activar agencia de carga',
            mensaje: estado_agencia_carga == 1 ?
                '<div class="alert alert-danger text-center"><i class="fa fa-fw fa-exclamation-triangle"></i> ¿Está seguro de desactivar esta agencia de carga?</div>' :
        ...
        modal_quest('modal_actualizar_estado_agencia_carga', mensaje['mensaje'], mensaje['title'], true, false,
            '{{ isPC() ? '25%' : '' }}',
            function() {
                datos = {
                    _token: '{{ csrf_token() }}',
    ...
    function exportar_agencia_carga() {
        $.LoadingOverlay('show');
        window.open('{{ route('excel.agencias_carga') }}' + '?busqueda=' + $('#busqueda_agencias_carga').val().trim(),
            '_blank');
        $.LoadingOverlay('hide');
    }
</script>
