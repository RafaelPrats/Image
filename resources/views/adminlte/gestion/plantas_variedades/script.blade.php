<script>
    ...
    function add_planta() {
        ...
        $.get('{{ url('plantas_variedades/add_planta') }}', {}, function(retorno) {
            modal_form('modal_add_planta', retorno, '<i class="fa fa-fw fa-plus"></i> Añadir tipo', true, false,
                '{{ isPC() ? '65%' : '' }}',
                function() {
                    store_planta();
                });
    ...
    function add_variedad() {
        ...
        $.get('{{ url('plantas_variedades/add_variedad') }}', {}, function(retorno) {
            modal_form('modal_add_variedad', retorno, '<i class="fa fa-fw fa-plus"></i> Añadir tipo', true,
                false, '{{ isPC() ? '75%' : '' }}',
                function() {
                    store_variedad();
                });
    ...
    function edit_planta(p) {
        ...
        $.get('{{ url('plantas_variedades/edit_planta') }}', datos, function(retorno) {
            modal_form('modal_edit_planta', retorno, '<i class="fa fa-fw fa-plus"></i> Editar variedad', true,
                false, '{{ isPC() ? '65%' : '' }}',
                function() {
                    update_planta();
                });
    ...
    function edit_variedad(v) {
        ...
        $.get('{{ url('plantas_variedades/edit_variedad') }}', datos, function(retorno) {
            modal_form('modal_edit_variedad', retorno, '<i class="fa fa-fw fa-plus"></i> Editar tipo', true,
                false, '{{ isPC() ? '75%' : '' }}',
                function() {
                    update_variedad();
                });
    ...
    function update_variedad() {
        if ($('#form_edit_variedad').valid()) {
            ...
            datos = {
                ...
                tallos_x_malla: $('#tallos_x_malla').val(),
                saldo_inicial: $('#saldo_inicial').val(),
                desecho_propag_disponibilidad: $('#desecho_propag_disponibilidad').val(),
                cant_semanas_propag: $('#cant_semanas_propag').val(),
                orden: $('#orden').val(),
                receta: $('#receta').prop('checked'),
            };
    ...
    function distribuciones(p) {
        datos = {
            planta: p
        };
        get_jquery('{{ url('plantas_variedades/distribuciones') }}', datos, function(retorno) {
            modal_view('modal_distribuciones', retorno,
                '<i class="fa fa-fw fa-filter"></i> Distribuciones', true, false, '95%');
        });
    }
</script>
