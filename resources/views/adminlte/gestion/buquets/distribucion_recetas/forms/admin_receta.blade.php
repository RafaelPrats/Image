...
<input type="hidden" id="id_det_ped_seleccionado" value="{{ $det_ped->id_detalle_pedido }}">
<input type="hidden" id="id_det_esp_seleccionado" value="{{ $det_esp->id_detalle_especificacionempaque }}">
            ...
            <div style="overflow-y: scroll; overflow-x: scroll; max-height: 500px;">
                <table class="table-striped table-bordered" style="width: 100%; border: 1px solid #9d9d9d"
                    ...
                    @php
                        $pos = 0;
                    @endphp
                    @foreach ($item_recetas as $pos => $item)
                        <tr id="tr_variedad_seleccionado_{{ $pos + 1 }}">
                            ...
                            <td class="text-center" style="border-color: #9d9d9d">
                                ...
                                <input type="hidden" id="id_planta_seleccionado_{{ $pos + 1 }}"
                            ...
                            <td class="text-center" style="border-color: #9d9d9d">
                                <input type="number" class="text-center" style="width: 100%"
                                    ...
                                    value="{{ $item->tallos / ($ramos_x_caja * $piezas) }}">
                            </td>
                            <td class="text-center" style="border-color: #9d9d9d">
...
<script>
        ...
        for (i = 0; i < variedades_listados.length; i++) {
            ...
            if ($('#cantidad_' + id).val() > 0) {
                ...
                $('#table_variedades_seleccionados').append('<tr id="tr_variedad_seleccionado_' +
                    ...
                    '" value="' + id + '">' +
                    '<input type="hidden" id="id_planta_seleccionado_' + cant_variedad_seleccionado +
                    '" value="' + planta + '">' +
                    '</td>' +
                    '<td class="text-center" style="border-color: #9d9d9d">' +
    ...
    function store_agregar_variedades() {
        ...
        for (i = 0; i < cant_variedad_seleccionado.length; i++) {
            ...
            id_planta = $('#id_planta_seleccionado_' + pos).val();
            det_ped = $('#id_det_ped_seleccionado').val();
            det_esp = $('#id_det_esp_seleccionado').val();
        ...
        if (data.length > 0) {
            ...
            post_jquery_m('{{ url('distribucion_recetas/store_agregar_variedades') }}', datos, function(retorno) {
                cerrar_modals();
                listar_reporte();
            });
        }
