<div style="overflow-x: scroll; width: 100%">
    <table class="table-bordered" style="width: 100%; border-radius: 18px 18px 0 0" id="tabla_disponibilidades">
            ...
            @foreach($listado as $item)
                ...
                <td class="text-center td_yura_default" style="border-color: #9d9d9d; background-color: #e9ecef">
                    <table class="table-bordered" style="width: 100%">
                        <tr>
                            <td class="text-center td_yura_default" style="border-color: #9d9d9d; width: 50%">
                                ...
                                <input type="hidden" id="plantas_sembradas_{{$item->id_propag_disponibilidad}}" style="width: 100%"
                                       value="{{$item->plantas_sembradas}}">
                            </td>
                            <td class="text-center" style="border-color: #9d9d9d">
                                <input type="number" required style="width: 100%" value="0" class="text-center bg-yura_dark mouse-hand"
                                       id="cambiar_ptas_sembradas_{{$item->id_propag_disponibilidad}}"
                                       title="Doble click para cambiar semanas de ptas sembradas"
                                       ondblclick="cambiar_ptas_sembradas('{{$item->id_propag_disponibilidad}}')">
                            </td>
                        </tr>
                    </table>
                </td>
        ...
        <tr>
            <th class="text-center th_yura_green columna_fija_1" rowspan="2">Requerimientos</th>
            @foreach($listado as $item)
                <td class="text-center" style="border-color: #9d9d9d; vertical-align: text-top" valign="top">
                    @include('adminlte.gestion.propagacion.disponibilidad.partials._requerimientos')
                </td>
            @endforeach
        </tr>
        <tr>
                ...
                <td class="text-center td_yura_default" style="border-color: #9d9d9d">
                    <table class="table-bordered" style="width: 100%">
                        <tr>
                            ...
                            <td class="text-center" style="border-color: #9d9d9d; background-color: #e9ecef">
                                {{number_format($item->calcular_requerimientos())}}
                            </td>
                            <td class="text-center" style="border-color: #9d9d9d; background-color: #e9ecef">
...
<script>
    ...
    function update_disponibilidad(id, mantener_cambios) {
        ...
        datos = {
            ...
            req_adicionales: req_adicionales,
    ...
    function cambiar_ptas_sembradas(id_disp) {
        datos = {
            id_disp: id_disp,
        ...
        if (datos['valor'] != 0)
            modal_quest('modal-quest_cambiar_ptas_sembradas',
                '<div class="alert alert-warning text-center">¿Desea CAMBIAR las semanas de maduración?</div>',
