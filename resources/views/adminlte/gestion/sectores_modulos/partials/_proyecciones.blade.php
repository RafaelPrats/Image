@if(count($modulos) > 0)
    <div style="overflow-x: scroll; max-height: 450px">
        <table class="table-striped table-bordered" width="100%" style="border: 1px solid #9d9d9d; border-radius: 18px 18px 0 0"
            ...
            <tr>
                ...
                <th class="text-center" style="border-color: white; color: white; background-color: #00b388" colspan="{{$tipo == 1 ? 14 : 9}}">
                    Ciclos
                </th>
            ...
            @foreach($modulos as $pos_mdl => $modulo)
                ...
                <tr>
                    ...
                    <td class="text-center" style="border-color: #9d9d9d">
                        <select name="tipo_planta_{{$modulo->id_modulo}}" id="tipo_planta_{{$modulo->id_modulo}}" style="width: 100%"
                                class="input-yura_default">
                            <option value="S" {{$tipo == 1 && $cicloActual->tipo_planta == 'S' ? 'selected' : ''}}>S</option>
                            <option value="G" {{$tipo == 1 && $cicloActual->tipo_planta == 'G' ? 'selected' : ''}}>G</option>
                        </select>
                    </td>
                    @if($tipo == 1)
                        <td class="text-center" style="border-color: #9d9d9d">
                    ...
                    <td class="text-center" style="border-color: #9d9d9d">
                        @php
                            if ($tipo == 1){
                                $area = $cicloActual->area;
                            } else {
                                $area = $getLastCiclo != '' ? $getLastCiclo->area : $modulo->area;
                            }
                            $total_area += $area;
                        @endphp
                        ...
                        <input type="number" id="ciclo_area_{{$modulo->id_modulo}}" name="ciclo_area_{{$modulo->id_modulo}}"
                               class="text-center input-yura_white" value="{{$area}}"
                    ...
                    <td class="text-center" style="border-color: #9d9d9d">
                        @php
                            if ($tipo == 1){
                                $plantas_muertas = $cicloActual != '' ? $cicloActual->plantas_muertas : 0;
                            } else {
                                $plantas_muertas = $getLastCiclo != '' ? $getLastCiclo->plantas_muertas : 0;
                            }
                            $total_muertas += $plantas_muertas;
                        @endphp
            ...
            <tr>
                <th class="text-center" style="border-color: #9d9d9d" colspan="{{$tipo == 1 ? 10 : 7}}">
                    Total
                </th>
                <th class="text-center" style="border-color: #9d9d9d">
