    ...
    <div id="div_content_fixed">
        <table class="table-striped table-bordered" width="100%" style="border: 1px solid #9d9d9d; border-radius: 18px 18px 0 0;"
            ...
            @foreach($ciclos as $pos_item => $c)
                ...
                @endphp
                <tr style="background-color: {{$pos_item % 2 == 0 ? '#72ffe052' : ''}}">
                    ...
                    <td class="text-center" style="border-color: #9d9d9d">
                        {{number_format($getPlantasProductivas)}}
                    </td>
                    <td class="text-center" style="border-color: #9d9d9d">
                        {{number_format($getExquejesCosechados)}}
                    </td>
                    <td class="text-center" style="border-color: #9d9d9d">
                    ...
                    <td class="text-center"
                        style="border-color: #9d9d9d; color: {{$semana_fin != '' && $semana_fin->codigo <= $semana_actual->codigo ? '#D01C62' : ($semana_fin != '' && $semana_fin->codigo > $semana_actual->codigo && $semana_fin->codigo <= $semana_5_futuro->codigo ? '#EF6E11' : '#00B388')}}">
                    @if($semana_fin != '')
                            {{$semana_fin->codigo}}
                        @else
